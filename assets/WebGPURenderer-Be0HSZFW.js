import { a8 as L, e as A, P as l, a9 as D, K as ce, aa as de, a0 as B, D as C, J as T, H as he, ab as k, ac as w, y as v, ad as y, $ as pe, ae as le, af as z, l as M, O as F, ag as O, ah as fe, ai as I, aj as ge, S as W, h as R, a1 as me, a2 as _e, a3 as S } from "./index-v4uW8J1D.js";
import { c as be, u as ye, U as xe, B as Ge, G as Pe, e as Be, l as Se, a as Te, t as Ce } from "./BufferResource-DpOqAvDO.js";
import { R as ve, S as Ue, d as we } from "./RenderTargetSystem-C8RlhfJy.js";
...
class H {
  start(e, t, r) {
    ...
    const a = s.globalUniforms.bindGroup;
    i.resetBindGroup(1), i.setBindGroup(0, a, n);
  }
  execute(e, t) {
    ...
    const n = s.bindGroup.getBindGroup(t.bindGroup, r, 1), a = s.pipeline.getPipeline(this._geometry, r, G, t.topology);
    t.bindGroup._touch(s.gc.now, s.tick), i.setPipeline(a), i.renderPassEncoder.setBindGroup(1, n), i.renderPassEncoder.drawIndexed(t.size, 1, t.start);
  }
}
...
class V {
  ...
  _createBindGroup(e, t, r) {
    const s = this._gpu.device, i = t.layout[r], n = [], a = this._renderer;
    for (const g in i) {
      ...
      if (h._resourceType === "uniformGroup") {
        ...
        const f = d.buffer;
        p = { buffer: a.buffer.getGPUBuffer(f), offset: 0, size: f.descriptor.size };
      } else if (h._resourceType === "buffer") {
        const d = h;
        p = { buffer: a.buffer.getGPUBuffer(d), offset: 0, size: d.descriptor.size };
      } else if (h._resourceType === "bufferResource") {
        const d = h;
        p = { buffer: a.buffer.getGPUBuffer(d.buffer), offset: d.offset, size: d.size };
      } else if (h._resourceType === "textureSampler") {
        const d = h;
        p = a.texture.getGpuSampler(d);
      } else if (h._resourceType === "textureSource") {
        const d = h;
        p = a.texture.getTextureView(d);
      }
      n.push({ binding: i[g], resource: p });
    }
    const u = a.shader.getProgramData(t).bindGroups[r], c = s.createBindGroup({ layout: u, entries: n });
...
class K {
  ...
  draw(e) {
    const { geometry: t, shader: r, state: s, topology: i, size: n, start: a, instanceCount: u, skipSync: c } = e;
    this.setPipelineFromGeometryProgramAndState(t, r.gpuProgram, s, i), this.setGeometry(t, r.gpuProgram), this._setShaderBindGroups(r, c), t.indexBuffer ? this.renderPassEncoder.drawIndexed(n || t.indexBuffer.data.length, u ?? t.instanceCount, a || 0) : this.renderPassEncoder.draw(n || t.getSize(), u ?? t.instanceCount, a || 0);
  }
  finishRenderPass() {
    this.renderPassEncoder && (this.renderPassEncoder.end(), this.renderPassEncoder = null);
  }
  ...
  restoreRenderPass() {
    ...
    for (const a in i) this.setBindGroup(a, i[a], null);
...
$.extension = { type: [l.WebGPUSystem], name: "stencil" };
...
function Ee(o) {
  const e = o.map((r) => ({ data: r, offset: 0, size: 0 }));
  let t = 0;
...
function Le(o, e) {
  const { size: t, align: r } = P[o.data.type], s = (r - t) / 4, i = o.data.type.indexOf("i32") >= 0 ? "dataInt32" : "data";
  return `
         v = uv.${o.data.name};
         ${e !== 0 ? `offset += ${e};` : ""}
...
Y.extension = { type: [l.WebGPUSystem], name: "ubo" };
const b = 128;
class X {
  constructor(e) {
...
function ke(o, e, t, r, s) {
  return o << 24 | e << 16 | t << 10 | r << 5 | s;
}
function ze(o, e, t, r, s) {
  return t << 8 | o << 5 | r << 3 | s << 1 | e;
}
class J {
  constructor(e) {
  ...
  _createPipeline(e, t, r, s) {
    const i = this._gpu.device, n = this._createVertexBufferLayouts(e, t), a = this._renderer.state.getColorTargets(r, this._colorTargetCount), u = this._stencilMode === C.RENDERING_MASK_ADD ? 0 : this._colorMask;
    for (let p = 0; p < a.length; p++) a[p].writeMask = u;
    const c = this._renderer.shader.getProgramData(t).pipeline, g = { vertex: { module: this._getModule(t.vertex.source), entryPoint: t.vertex.entryPoint, buffers: n }, fragment: { module: this._getModule(t.fragment.source), entryPoint: t.fragment.entryPoint, targets: a }, primitive: { topology: s, cullMode: r.cullMode }, layout: c, multisample: { count: this._multisampleCount }, label: "PIXI Pipeline" };
  ...
  _generateBufferKey(e) {
    ...
    for (let n = 0; n < s.length; n++) {
      const a = e.attributes[s[n]];
      t[r++] = a.offset, t[r++] = a.format, t[r++] = a.stride, t[r++] = a.instance;
    }
    const i = t.join("|");
  ...
  getBufferNamesToBind(e, t) {
    ...
    for (let a = 0; a < s.length; a++) {
      const c = Object.values(s[a].attributes)[0].shaderLocation;
  ...
  _createVertexBufferLayouts(e, t) {
    ...
    return e.buffers.forEach((i) => {
      const n = { arrayStride: 0, stepMode: "vertex", attributes: [] }, a = n.attributes;
      for (const u in t.attributeData) {
        const c = e.attributes[u];
        (c.divisor ?? 1) !== 1 && v(`Attribute ${u} has an invalid divisor value of '${c.divisor}'. WebGPU only supports a divisor value of 1`), c.buffer === i && (n.arrayStride = c.stride, n.stepMode = c.instance ? "instance" : "vertex", a.push({ shaderLocation: t.attributeData[u].location, offset: c.offset, format: c.format }));
      }
      a.length && s.push(n);
...
J.extension = { type: [l.WebGPUSystem], name: "pipeline" };
class Fe {
  constructor() {
...
class Oe {
  ...
  copyToTexture(e, t, r, s, i) {
    const n = this._renderer, a = this._getGpuColorTexture(e), u = n.texture.getGpuSource(t.source);
    return n.encoder.commandEncoder.copyTextureToTexture({ texture: a, origin: r }, { texture: u, origin: i }, s), t;
  }
  ...
  getDescriptor(e, t, r, s = 0, i = 0) {
    ...
    const n = this._renderTargetSystem, a = n.getGpuRenderTarget(e), u = e.colorTextures.map((h, p) => {
      const d = a.contexts[p];
      let f, m;
      if (d) {
      ...
      a.msaaTextures[p] && (m = f, f = this._renderer.texture.getTextureView(a.msaaTextures[p]));
      const x = t & y.COLOR ? "clear" : "load";
      return r ?? (r = n.defaultClearColor), { view: f, resolveTarget: m, clearValue: r, storeOp: "store", loadOp: x };
    });
    let c;
    if ((e.stencil || e.depth) && !e.depthStencilTexture && (e.ensureDepthStencilTexture(), e.depthStencilTexture.source.sampleCount = a.msaa ? 4 : 1), e.depthStencilTexture) {
  ...
  clear(e, t = true, r, s, i = 0, n = 0) {
    if (!t) return;
    const { gpu: a, encoder: u } = this._renderer, c = a.device;
...
Z.extension = { type: [l.WebGPUSystem], name: "renderTarget" };
class Q {
  constructor() {
...
Q.extension = { type: [l.WebGPUSystem], name: "shader" };
const _ = {};
...
const Ie = { type: "image", upload(o, e, t, r = 0) {
  const s = o.resource, i = (o.pixelWidth | 0) * (o.pixelHeight | 0), n = s.byteLength / i;
  t.device.queue.writeTexture({ texture: e, origin: { x: 0, y: 0, z: r } }, s, { offset: 0, rowsPerImage: o.pixelHeight, bytesPerRow: o.pixelWidth * n }, { width: o.pixelWidth, height: o.pixelHeight, depthOrArrayLayers: 1 });
} }, te = { "bc1-rgba-unorm": { blockBytes: 8, blockWidth: 4, blockHeight: 4 }, "bc2-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "bc3-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "bc7-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "etc1-rgb-unorm": { blockBytes: 8, blockWidth: 4, blockHeight: 4 }, "etc2-rgba8unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "astc-4x4-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 } }, We = { blockBytes: 4, blockWidth: 1, blockHeight: 1 }, He = { type: "compressed", upload(o, e, t, r = 0) {
  let s = o.pixelWidth, i = o.pixelHeight;
  const n = te[o.format] || We;
  for (let a = 0; a < o.resource.length; a++) {
    const u = o.resource[a], c = Math.ceil(s / n.blockWidth) * n.blockBytes;
    t.device.queue.writeTexture({ texture: e, mipLevel: a, origin: { x: 0, y: 0, z: r } }, u, { offset: 0, bytesPerRow: c }, { width: Math.ceil(s / n.blockWidth) * n.blockWidth, height: Math.ceil(i / n.blockHeight) * n.blockHeight, depthOrArrayLayers: 1 }), s = Math.max(s >> 1, 1), i = Math.max(i >> 1, 1);
  }
...
function Ve(o) {
  return { type: "cube", upload(e, t, r) {
    ...
    for (let i = 0; i < E.length; i++) {
      const n = E[i], a = s[n];
      (o[a.uploadMethodId] || o.image).upload(a, t, r, i);
    }
  } };
}
const re = { type: "image", upload(o, e, t, r = 0) {
  const s = o.resource;
  if (!s) return;
  if (globalThis.HTMLImageElement && s instanceof HTMLImageElement) {
    const u = B.get().createCanvas(s.width, s.height);
    u.getContext("2d").drawImage(s, 0, 0, s.width, s.height), o.resource = u, v("ImageSource: Image element passed, converting to canvas and replacing resource.");
  }
  const i = Math.min(e.width, o.resourceWidth || o.pixelWidth), n = Math.min(e.height, o.resourceHeight || o.pixelHeight), a = o.alphaMode === "premultiply-alpha-on-upload";
  t.device.queue.copyExternalImageToTexture({ source: s }, { texture: e, origin: { x: 0, y: 0, z: r }, premultipliedAlpha: a }, { width: i, height: n });
} }, Ne = { type: "video", upload(o, e, t, r) {
  re.upload(o, e, t, r);
} };
class je {
  constructor(e) {
  ...
  generateMipmap(e) {
    ...
    const n = this.device.createCommandEncoder({}), a = t.getBindGroupLayout(0);
    for (let u = 0; u < s; ++u) {
      ...
      for (let h = 1; h < e.mipLevelCount; ++h) {
        const p = r.createView({ baseMipLevel: g++, mipLevelCount: 1, dimension: "2d", baseArrayLayer: u, arrayLayerCount: 1 }), d = n.beginRenderPass({ colorAttachments: [{ view: p, storeOp: "store", loadOp: "clear", clearValue: { r: 0, g: 0, b: 0, a: 0 } }] }), f = this.device.createBindGroup({ layout: a, entries: [{ binding: 0, resource: this.sampler }, { binding: 1, resource: c }] });
...
class se {
  ...
  getPixels(e) {
    ...
    s.drawImage(t, 0, 0);
    const { width: i, height: n } = t, a = s.getImageData(0, 0, i, n), u = new Uint8ClampedArray(a.data.buffer);
...
class ie {
  ...
  execute(e, t) {
    const r = t.context, s = r.customShader || this.shader, i = e.renderer, n = i.graphicsContext, { batcher: a, instructions: u } = n.getContextRenderData(r), c = i.encoder;
    c.setGeometry(a.geometry, s.gpuProgram);
    ...
    for (let f = 0; f < u.instructionSize; f++) {
      const m = p[f];
      if (m.topology !== d && (d = m.topology, c.setPipelineFromGeometryProgramAndState(a.geometry, s.gpuProgram, e.state, m.topology)), s.groups[1] = m.bindGroup, !m.gpuBindGroup) {
        const x = m.textures;
...
class ne {
  init() {
    ...
    this._shader = new W({ gpuProgram: e, resources: { uTexture: R.EMPTY._source, uSampler: R.EMPTY._source.style, textureUniforms: { uTextureMatrix: { type: "mat3x3<f32>", value: new F() } } } });
  }
  execute(e, t) {
    const r = e.renderer;
    let s = t._shader;
...
const qe = [...Ue, Y, K, U, q, N, se, Z, Q, ee, J, j, $, V], $e = [...we, X], Ye = [H, ne, ie], ae = [], oe = [], ue = [];
S.handleByNamedList(l.WebGPUSystem, ae);
S.handleByNamedList(l.WebGPUPipes, oe);
S.handleByNamedList(l.WebGPUPipesAdaptor, ue);
S.add(...qe, ...$e, ...Ye);
class rt extends me {
  constructor() {
    const e = { name: "webgpu", type: _e.WEBGPU, systems: ae, renderPipes: oe, renderPipeAdaptors: ue };
    super(e);
  }
}
export {
