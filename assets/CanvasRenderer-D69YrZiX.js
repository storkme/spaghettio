import { z as x, q as Ce, A as Te, D as z, M as B, F as _, H as Ue, w as qe, J as Le, h as Pe, T as ke, K as Oe, L as Ne, N as je, O as Ve, P as Ye, Q as Xe, U as be, V as $e, W as Je, X as ee } from "./index-D2f19UvT.js";
import { R as Ke, S as Qe, B as ze, a as Ze, b as et, c as tt, A as st, C as nt } from "./RenderTargetSystem-NUVyvph8.js";
import "./Filter-B-4b-lvw.js";
const he = class q {
  ...
  execute(e, t) {
    ...
    for (let c = 0; c < n.length; c++) {
      ...
      const u = d, l = u.texture, T = l ? x.getCanvasSource(l) : null;
      if (!T) continue;
      const y = l.source.style, V = o.smoothProperty, E = y.scaleMode !== "nearest";
      r[V] !== E && (r[V] = E), o.setBlendMode(t.blendMode);
      ...
      if (W <= 0) continue;
      r.globalAlpha = W;
      const $ = H & 16777215, k = P & 16777215, F = Te(z(k, $)), R = l.frame, Y = y.addressModeU ?? y.addressMode, L = y.addressModeV ?? y.addressMode, p = q._getPatternRepeat(Y, L), m = l.source._resolution ?? l.source.resolution ?? 1, f = (_d = (_c = u.renderable) == null ? void 0 : _c.renderGroup) == null ? void 0 : _d.isCachedAsTexture, G = R.x * m, O = R.y * m, b = R.width * m, D = R.height * m, v = u.bounds, S = a.renderTarget.renderTarget.isRoot, M = v.minX, X = v.minY, U = v.maxX - v.minX, A = v.maxY - v.minY, C = l.rotate, i = l.uvs, te = Math.min(i.x0, i.x1, i.x2, i.x3, i.y0, i.y1, i.y2, i.y3), Ee = Math.max(i.x0, i.x1, i.x2, i.x3, i.y0, i.y1, i.y2, i.y3), de = p !== "no-repeat" && (te < 0 || Ee > 1), se = C && !(!de && (F !== 16777215 || C));
      se ? (q._tempPatternMatrix.copyFrom(u.transform), Ce.matrixAppendRotationInv(q._tempPatternMatrix, C, M, X, U, A), o.setContextTransform(q._tempPatternMatrix, u.roundPixels === 1, void 0, f && S)) : o.setContextTransform(u.transform, u.roundPixels === 1, void 0, f && S);
      const J = se ? 0 : M, K = se ? 0 : X, ne = U, ae = A;
      if (de) {
        let oe = T;
        const N = F !== 16777215 && !C, j = R.width <= l.source.width && R.height <= l.source.height;
        N && j && (oe = x.getTintedCanvas({ texture: l }, F));
        const re = r.createPattern(oe, p);
        if (!re) continue;
        const pe = ne, ue = ae;
        if (pe === 0 || ue === 0) continue;
        const me = 1 / pe, fe = 1 / ue, ge = (i.x1 - i.x0) * me, ve = (i.y1 - i.y0) * me, xe = (i.x3 - i.x0) * fe, ye = (i.y3 - i.y0) * fe, We = i.x0 - ge * J - xe * K, Fe = i.y0 - ve * J - ye * K, ie = l.source.pixelWidth, ce = l.source.pixelHeight;
        q._tempPatternMatrix.set(ge * ie, ve * ce, xe * ie, ye * ce, We * ie, Fe * ce), x.applyPatternTransform(re, q._tempPatternMatrix), r.fillStyle = re, r.fillRect(J, K, ne, ae);
      } else {
        const N = F !== 16777215 || C ? x.getTintedCanvas({ texture: l }, F) : T, j = N !== T;
        r.drawImage(N, j ? 0 : G, j ? 0 : O, j ? N.width : b, j ? N.height : D, J, K, ne, ae);
      }
    }
  }
};
he._tempPatternMatrix = new B();
he.extension = { type: [_.CanvasPipesAdaptor], name: "batch" };
let at = he;
class _e {
  constructor(e) {
...
_e.extension = { type: [_.CanvasPipes], name: "colorMask" };
...
Me.extension = { type: [_.CanvasPipes], name: "stencilMask" };
const h = "source-over";
function it() {
...
we.extension = { type: [_.CanvasSystem], name: "canvasContext" };
class Ie {
  constructor() {
...
Ie.extension = { type: [_.CanvasSystem], name: "limits" };
const lt = "#808080", Q = new B(), ht = new B(), dt = new B(), le = new B();
function pt(s, e, t) {
  s.beginPath();
...
function gt(s, e, t, n) {
  ...
  if (a instanceof Ve) {
    ...
    const r = a.texture;
    if (r) {
      const c = x.getTintedPattern(r, e), d = t ? le.copyFrom(t).scale(r.source.pixelWidth, r.source.pixelHeight) : le.copyFrom(a.transform);
      return n && !s.textureSpace && d.append(n), x.applyPatternTransform(c, d), c;
    }
  }
  if (a instanceof Ye) {
    const r = x.getTintedPattern(a.texture, e);
    return x.applyPatternTransform(r, a.transform), r;
  }
  const o = s.texture;
  if (o && o !== ke.WHITE) {
    if (!o.source.resource) return lt;
    const r = x.getTintedPattern(o, e), c = t ? le.copyFrom(t).scale(o.source.pixelWidth, o.source.pixelHeight) : s.matrix;
    return x.applyPatternTransform(r, c), r;
  }
  return ut(e);
}
class Re {
  ...
  execute(e, t) {
    ...
    const n = e.renderer, a = n.canvasContext, o = a.activeContext, r = t.groupTransform, c = ((_a = n.globalUniforms.globalUniformData) == null ? void 0 : _a.worldColor) ?? 4294967295, d = t.groupColorAlpha, u = (c >>> 24 & 255) / 255, l = (d >>> 24 & 255) / 255, T = ((_b = n.filter) == null ? void 0 : _b.alphaMultiplier) ?? 1, y = u * l * T;
    if (y <= 0) return;
    const V = c & 16777215, E = d & 16777215, H = Te(z(E, V)), P = n._roundPixels | t._roundPixels;
    o.save(), a.setContextTransform(r, P === 1), a.setBlendMode(t.groupBlendMode);
    const w = t.context.instructions;
    for (let g = 0; g < w.length; g++) {
      const I = w[g];
      if (I.action === "texture") {
        const p = I.data, m = p.image, f = m ? x.getCanvasSource(m) : null;
        if (!f) continue;
        const G = p.alpha * y;
        if (G <= 0) continue;
        const O = z(p.style, H);
        o.globalAlpha = G;
        let b = f;
        O !== 16777215 && (b = x.getTintedCanvas({ texture: m }, O));
        const D = m.frame, v = m.source._resolution ?? m.source.resolution ?? 1;
        let S = D.x * v, M = D.y * v;
        const X = D.width * v, U = D.height * v;
        b !== f && (S = 0, M = 0);
        const A = p.transform, C = A && !A.isIdentity(), i = m.rotate;
        C || i ? (Q.copyFrom(r), C && Q.append(A), i && Ce.matrixAppendRotationInv(Q, i, p.dx, p.dy, p.dw, p.dh), a.setContextTransform(Q, P === 1)) : a.setContextTransform(r, P === 1), o.drawImage(b, S, M, b === f ? X : b.width, b === f ? U : b.height, i ? 0 : p.dx, i ? 0 : p.dy, p.dw, p.dh), (C || i) && a.setContextTransform(r, P === 1);
        continue;
      }
      const W = I.data, $ = (_c = W == null ? void 0 : W.path) == null ? void 0 : _c.shapePath;
      if (!((_d = $ == null ? void 0 : $.shapePrimitives) == null ? void 0 : _d.length)) continue;
      const k = W.style, F = z(k.color, H), R = k.alpha * y;
      if (R <= 0) continue;
      const Y = I.action === "stroke";
      if (o.globalAlpha = R, Y) {
        const p = k;
        o.lineWidth = p.width, o.lineCap = p.cap, o.lineJoin = p.join, o.miterLimit = p.miterLimit;
      }
      const L = $.shapePrimitives;
      if (!Y && ((_g = (_f = (_e2 = W.hole) == null ? void 0 : _e2.shapePath) == null ? void 0 : _f.shapePrimitives) == null ? void 0 : _g.length)) {
      ...
      for (let p = 0; p < L.length; p++) {
        ...
        const f = m.transform, G = f && !f.isIdentity(), O = k.texture && k.texture !== ke.WHITE, b = k.textureSpace === "global" ? f : null, D = O ? Oe(ht, k, m.shape, b) : null, v = G ? dt.copyFrom(r).append(f) : r, S = gt(k, F, D, v);
        if (G && (o.save(), o.transform(f.a, f.b, f.c, f.d, f.tx, f.ty)), Y) {
          const M = k;
          if (M.alignment !== 0.5 && !M.pixelLine) {
            const U = [], A = [], C = [];
            if ((_h = Ne[m.shape.type]) == null ? void 0 : _h.build(m.shape, U)) {
              const te = m.shape.closePath ?? true;
              je(U, M, false, te, A, C), o.fillStyle = S, pt(o, A, C);
...
Re.extension = { type: [_.CanvasPipesAdaptor], name: "graphics" };
class vt {
  init(e, t) {
  ...
  _ensureCanvas(e) {
    let t = e.resource;
    (!t || !Xe.test(t)) && (t = be.get().createCanvas(e.pixelWidth, e.pixelHeight), e.resource = t), (t.width !== e.pixelWidth || t.height !== e.pixelHeight) && (t.width = e.pixelWidth, t.height = e.pixelHeight);
...
Ae.extension = { type: [_.CanvasSystem], name: "renderTarget" };
class Be {
  constructor(e) {
  }
  init() {
  ...
  generateCanvas(e) {
    const t = be.get().createCanvas(), n = t.getContext("2d"), a = x.getCanvasSource(e);
    if (!a) return t;
...
Be.extension = { type: [_.CanvasSystem], name: "texture" };
const xt = [...Qe, we, Ie, Be, Ae], yt = [ze, Ze, et, tt, st, Me, _e, nt], Ct = [at, Re], He = [], Ge = [], De = [];
ee.handleByNamedList(_.CanvasSystem, He);
ee.handleByNamedList(_.CanvasPipes, Ge);
ee.handleByNamedList(_.CanvasPipesAdaptor, De);
ee.add(...xt, ...yt, ...Ct);
class _t extends $e {
  constructor() {
    const e = { name: "canvas", type: Je.CANVAS, systems: He, renderPipes: Ge, renderPipeAdaptors: De };
    super(e);
  }
}
export {
