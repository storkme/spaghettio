...
| `crates/core/src/validate/` | The 37 functional checks, dispatched from `mod.rs` (`belt_flow` lane-rate walker, `belt_structural`, `fluids`, `inserters`, `modules`, `power`, `underground`) |
...
| Validation checks | `crates/core/src/validate/` (37 checks, dispatched from `mod.rs`) |
