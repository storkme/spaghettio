...
1. **Solver** (`crates/core/src/solver.rs`) — Resolves recipe dependencies via the net-flow LP in `netflow.rs` (free cost-based recipe selection, default since 2026-07; the legacy recursive tree walk and its compat-mode A/B path were deleted 2026-08-14, #632 A1 — the walk-vs-LP comparison tests went with it, but netflow's own regression suite survived the split as `crates/core/tests/netflow_regression.rs`), computes machine counts and flow rates. Loads `crates/core/data/recipes.json` via `include_str!`. Returns a `SolverResult`.
...
| `crates/core/src/netflow.rs` | Net-flow LP solver — the only solver path since 2026-08-14 (#632 A1 deleted the legacy tree walk and its compat A/B mode in `solver.rs`; its own regression coverage lives in `crates/core/tests/netflow_regression.rs`); free cost-based recipe selection, byproduct crediting, typed cycle refusals. See `docs/rfc-solver-net-flow.md`. |
| `crates/core/src/validate/` | The 38 functional checks, dispatched from `mod.rs` (`belt_flow` lane-rate walker, `belt_structural`, `burner_fuel`, `fluids`, `inserters`, `modules`, `power`, `underground`) |
...
2. **Measure it, don't look at it** (revised 2026-08-08 — the browser eyeball used to be this step). The validator is not ground truth, but the eyeball was never a good substitute: it catches *disconnected*, not *underdelivering*, and the defects that survive validation are overwhelmingly the second kind. Two instruments now cover it properly:
   - **The fast meter** (`crates/meter/`, seconds) — `check_one <dir>` on an exported fixture. Its asymmetry WAS the calibrated one: *"meter says below plan ⇒ believe it"*; *"meter says at plan"* is evidence of nothing (the floor property does **not** hold post-lift — see [`meter-divergence.md`](docs/meter-divergence.md) §2026-08-08). **2026-08-25: the below-plan direction is also falsified for turn-heavy fixtures** (a 5–18pp phantom under-read; §2026-08-25 of the same log) — until the turn model is fixed, a below-plan meter reading on a layout whose item paths turn must be sim-anchored before any decision rides on it. The meter still refutes cheaply on capacity-bound claims and still clears nothing.
...
| Balancer templates | `crates/core/src/bus/balancer_library.rs`. Regenerate via either live pipeline: `python scripts/generate_balancer_library.py` (Factorio-SAT on `PATH`; drives `sync_balancer_to_rust.py` + `import_balancer`) or `crates/balancer-gen` (Clos composition via its own `scripts/place.py` — the canonical placer). A third, CP-SAT pipeline was deleted 2026-08-20 (#677). |
...
| Validation checks | `crates/core/src/validate/` (38 checks, dispatched from `mod.rs`) |
