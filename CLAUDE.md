...
- **Process docs**: non-trivial design work uses [`docs/rfp-template.md`](docs/rfp-template.md) — the **kill criteria** section is required, since the dominant rework shape on this project is exploration that overruns its evidence. PRs follow [`.github/pull_request_template.md`](.github/pull_request_template.md): when creating a PR (via `gh`, `mcp__github__create_pull_request`, or any other API/CLI path), **read the template file and fill each section explicitly** — it does not auto-populate the way it does in GitHub's web UI. Intent, scope, verification actually run, and any deviations from the agreed approach must all be present; "None" is a valid answer for a section, but leaving it blank is not. Trivial changes can omit sections explicitly rather than leaving them blank.
