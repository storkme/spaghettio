...
use crate::common::{
    ...
    is_splitter, is_surface_belt, is_ug_belt, machine_dims, machine_tiles,
    utilization_for,
};
#[cfg(test)]
...
/// (Since the 2026-08-21 check deletions, only the lane-budget TESTS
/// consume this wrapper — production sizing calls `common::belt_drop_rate`
/// directly — hence the cfg(test).)
#[cfg(test)]
...
// check_inserter_throughput and check_inserter_item_throughput were
// deleted 2026-08-21 (owner call, offpath Tier 2 item 9, #675): demoted
// from selection 2026-08-14 (#632 B6) as never-sim-anchored hand models,
// report-only since. Accepted cost recorded in status.md; trust-table
// rows tombstoned in the same PR.
...
mod tests {
    ...
    // (Deleted checks' test sections went with them — 2026-08-21.)
