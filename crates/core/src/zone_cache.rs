...
//! Native builds read/write to `$XDG_CACHE_HOME/spaghettio/sat-zones.bin`
//! (or `SPAGHETTIO_ZONE_CACHE_PATH` override); WASM builds load a pre-baked
...
/// `~/.cache/spaghettio/sat-zones.bin` to `crates/core/data/`.
...
/// the same run too.
static LOOKUP: OnceLock<Mutex<FxHashMap<String, CacheEntry>>> = OnceLock::new();

fn lookup_table() -> &'static Mutex<FxHashMap<String, CacheEntry>> {
    LOOKUP.get_or_init(|| {
        let mut map: FxHashMap<String, CacheEntry> = FxHashMap::default();
        #[cfg(not(target_arch = "wasm32"))]
...
fn install_prebaked_into(
    map: &mut FxHashMap<String, CacheEntry>,
    bytes: &[u8],
) {
...
#[cfg(not(target_arch = "wasm32"))]
fn load_existing_jsonl(map: &mut FxHashMap<String, CacheEntry>) {
...
/// `SPAGHETTIO_USE_ZONE_CACHE=0` to disable (e.g. when investigating a
...
fn cache_enabled() -> bool {
    ...
    {
        !matches!(
            std::env::var("SPAGHETTIO_USE_ZONE_CACHE").as_deref(),
...
fn channel_items_by_id(zone: &CrossingZone) -> FxHashMap<u32, String> {
    let mut map: FxHashMap<u32, String> = Default::default();
...
fn record_zone_outcome(
    ...
    let effective_source: Option<String> = ZONE_SOURCE
        ...
        .or_else(|| std::env::var("SPAGHETTIO_ZONE_SOURCE").ok());
    #[cfg(target_arch = "wasm32")]
