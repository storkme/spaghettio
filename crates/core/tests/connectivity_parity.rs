...
//!
//! Five compaction-transform-dependent tests were deleted the same day
//! (#632 A2, owner call) along with `bus::compaction` itself:
//! `phase2_prefilter_is_outcome_identical`,
//! `phase2_prefilter_identity_on_ug_normalizing_cut` (the K65-5
//! adversarial-review "span-loss" false-positive fixture),
//! `phase2_prefilter_rejects_engage_on_head_on_creating_cut` (the
//! "head-on-creating cut" Error-certain-class fixture),
//! `phase2b_fold_prefilter_measurement` (ignored measurement probe), and
//! `compaction_remaps_effective_rows` (the RFC-065 `effective_rows`-remap
//! regression pin). All five tested `compact_validated_geometry`'s/
//! `search_snake_fold`'s OWN internal admission-filter correctness, not
//! K65-1/K65-2 in general — with the transform gone there is no subject
//! left to pin. K65-1/K65-2 coverage on every non-compaction path (the
//! `parity_*`/`detection_*`/`dispatched_validate_*` tests below,
//! including the exotic-row-kind pins `parity_kovarex_self_loop` and
//! `parity_uranium_voider`) is untouched.
