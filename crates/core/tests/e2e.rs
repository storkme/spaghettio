...
// ---------------------------------------------------------------------------
// Junction regression: (21,161) corridor-template × region-solver interaction
// ---------------------------------------------------------------------------
//
// The advanced-circuit @5/s from-ore AM3 layout produces `belt-item-isolation`
// errors around (21,161) and (24,155). The pattern: the corridor template
// pre-pass in `ghost_router.rs` bridges a horizontal spec over one short run
// of vertical trunks (e.g. 2 plastic-bar trunks), stamps a permanent UG pair
// at the endpoints, then leaves a second adjacent run (e.g. 3 iron-plate
// trunks) for the region solver — which has nowhere to put its own UG-out
// because the Template claim is non-releasable. Result: the horizontal
// spec's UG-out feeds into a foreign-item surface belt.
//
// Region-fixture tests can't cover this because the failure is an
// INTERACTION between the corridor template and the region solver — the
// region solver's replay sees the constrained state but the Template claim
// was set by an earlier step the fixture doesn't replay.
//
// Marked `#[ignore]`: failing today. Make green by redesigning the pipeline
// so clustering happens BEFORE the corridor template and each cluster picks
// its own resolver (see `docs/walker-veto-debugging.md`).
#[test]
#[ignore]
#[ntest::timeout(30000)]
fn tier4_advanced_circuit_from_ore_am3_junction_21_161_no_errors() {
    let inputs: FxHashSet<String> = [
        "iron-ore", "copper-ore", "coal", "water", "crude-oil",
    ]
    .iter()
    ...
    let layout_result = layout::build_bus_layout(&solver_result, Some("transport-belt"))
        .unwrap_or_else(|e| panic!("layout: {e}"));

    let issues = match validate::validate(&layout_result, Some(&solver_result), LayoutStyle::Bus) {
        Ok(issues) => issues,
        Err(e) => e.issues,
    };

    // Bbox of the known failure zone + margin. Covers both the
    // (24,155) copper-cable / plastic-bar seam and the (24,161)
    // circuit / iron-plate seam — same failure mode, different items.
    let bbox_errors: Vec<&ValidationIssue> = issues
        .iter()
        .filter(|i| matches!(i.severity, Severity::Error))
        .filter(|i| {
            let x = i.x.unwrap_or(i32::MIN);
            let y = i.y.unwrap_or(i32::MIN);
            (18..=30).contains(&x) && (153..=168).contains(&y)
        })
        .collect();

    if !bbox_errors.is_empty() {
        let summary: Vec<String> = bbox_errors
            .iter()
            .map(|i| {
                format!(
                    "  [{:?}] ({:?},{:?}) {}: {}",
                    i.severity, i.x, i.y, i.category, i.message
                )
            })
            .collect();
        panic!(
            "expected zero validation errors around (21,161), got {}:\n{}",
            bbox_errors.len(),
            summary.join("\n")
        );
    }
