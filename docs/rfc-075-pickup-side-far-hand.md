...
**Status: COMPLETE 2026-08-27 — Phases 0–2 in one PR.** The mechanism
(E1) and the fix (E3) are both sim-receipted, `FAR_PICKUP_FACTOR = 0.85`
ships in the ladder, and every row it re-shaped was re-simmed at or
above its previous receipt (K75-3): ec@240 from ore 240.00/240 at
K=20, from plates 250/240, PU-from-ore 87.7% → 89.9%. Close-out and
residuals at the end of the decision log. Opened on RFC-073's replacement pointer for
...
- *2026-08-27 — Phase 2 shipped form, re-priced.* With the constant in
  ...
  **ac@56 is byte-identical again** (d2ed8119): under the gate the
  ...
  + PU-from-ore (bank).
- *2026-08-27 — the K=20 grid re-sims, first attempt.* Fixtures
  regenerated by `export_chain_fixtures_for_sim` (chain-ec240-d2:
  17,118 entities, 40 in / 20 out; chain-ec240p-d2: 40 in / 20 out).
  **From ore refused at pre-flight** by the harness's rig-vs-layout
  guard (#731): "feed rig for 'copper-ore' at (477,49) would place kit
  on layout tile (467,46)" — the tile is a bridge pole
  `repair_pole_network` stamped down the inter-strip clearance at
  x=467 (poles at y=22/31/40/46), ten tiles west of strip 2's
  copper-ore head. The K=24 grid's bridge happened to miss every head;
  K=20's copies are a different width and it does not. Nothing about
  the ladder — a composer/harness geometry clash the re-quantization
  exposed; the fix is to keep the bridge out of the rig lanes, which
  the composer knows (its interior boundary heads) without knowing the
  harness. **From plates timed out** at the derived 1095 s with three
  sims and two cargo builds sharing the CPU — re-run alone with
  `--timeout-secs 4000`.
- *2026-08-27 — the bridge keep-out.* The grid composer now computes
  `interior_rig_lanes` — the harness's own rig footprints around every
  interior boundary head, as the union over every stagger slot the
  harness might assign: a feed head's outward column plus, at each
  ladder depth (4/10/16/22/28), the 18-tile jog row to its WEST with
  the chest bank (±2 rows at jog 10–12) and the 2×2 kit at jog 15/18
  (`scenario.rs::feed_footprint`); a drain head's extension column
  with its ±2 band and the 2×2 kit at lateral +4/+7 over the whole
  clearance (`drain_footprint`). It runs the plain pole repair as a
  trial and, only when a trial bridge lands on one of those tiles,
  stamps one explicit bridge column per gap at the x with the most
  rig-free clearance rows, chained down the free rows at pitch ≤ 8
  (`stamp_bridge_columns`), then repairs again with the footprints
  kept out (`layout::repair_pole_network_with_keepout`); the reroute
  emits `CellGridBridgeRerouted { columns }`. Grids whose plain bridges
  never touched a footprint keep their geometry: **ac@56 stays
  d2ed8119**. A first cut used a solid ±22-tile lane per head instead
  of the footprints: it fixed the from-ore grid but on the from-plates
  grid (68-wide copies, three heads each) the lanes tiled the whole
  clearance, the column landed out of reach and the keep-out repair
  could place nothing — 265 `power` errors, strip 2 an island. The
  footprint version composes both grids error-free with one power
  network, pinned by `grid_bridge_poles_stay_out_of_interior_rig_lanes`
  (no pole on a rig tile, the event fired, zero power issues) and the
  two grid tests re-pinned to K=20 / `[10, 10]`. The harness pre-flight
  guard remains the oracle for the fixtures. Both grids re-simmed on
...
- *2026-08-27 — K75-3 on the bank row.* `tier5_processing_unit_from_ore_am3`
  ...
  **produced 1.80/2.00 (−10.1%), delivered −9.5%, WARN** — from
  ...
  what the tap leaves (the belt is its limit).
- *2026-08-27 — K75-3 on the registry rows: both K=20 grids PASS.*
  Fixtures regenerated by `export_chain_fixtures_for_sim` under the
  final composer; Factorio 2.0.77, 432k warmup, speed 32, both
  converged, kit clean. **ec@240 from ore** (2×10, explicit bridge
  column at x=1193, 17,118 entities, hash `818d7f52`): **produced
  240.00/240.00 (+0.0%)**, delivered +6.7%, 1,200 machines `working`,
  20 `item_ingredient_shortage` = one cable-row tail per copy (the
  same last-machine frame E3 showed; copper at exactly plan). **ec@240
  from plates** (2×10 of the same cell, no furnace stage, plain bridge
  — no reroute needed under the real footprints — hash `7ffba350`):
  **produced 250.00/240.00 (+4.2%)**, delivered +0.0%, all 240
  machines `working`. Against the rows they replace (K=24: +0.0% and
  +0.0%): no regression, 96 → 100 EC machines and 192 → 160 cable
  machines per grid (the 12/s cell is denser in EC and lighter in
  cable), 17,148 → 17,118 entities from ore. Registry rows re-blessed
  on the new hashes; K75-3 holds on all three re-shaped rows (bank PU
  +2.2 pts, both grids at/above plan).
- *2026-08-27 — suite on the re-blessed tree.* 32 binaries,
  no-fail-fast: 1,318 passed, 1 failed, 108 ignored. The one failure
  was `tier5_processing_unit_from_ore_am3`'s warning-pin golden —
  `input-rate-delivery` 10 → 13 on the re-shaped PU fixture (the same
  10 → 13 the bank matrix recorded; the second far hands give the
  walker three more inserter shares to compare against the tap-fed
  belts' flow, on rows whose tails the sim shows starving). Adjudicated
  as the geometry change this RFC makes, not a validator drift, and
  re-blessed; 1,319 / 0 after.
- *2026-08-27 — CLOSE-OUT: complete.* Against the Summary: (1) the
  mechanism is confirmed by the two sims that vary only the belt
  state — E1 flooded (+4.2%, all five hands at 2.50) vs E0 streaming
  (−7.1%, 2.11 → 2.48); (2) the fix ships as `FAR_PICKUP_FACTOR =
  0.85` in `inserter_ladder::far_pickup_rate`, the single source for
  `size_side` and `contest_favors_far`, input far sides only, with the
  composer's far-side `HAND_MARGIN` retired and the census re-pricer
  reading it; (3) priced and re-blessed — 1 bank row + 2 registry rows
  moved, every sub-K_MAX strip and ac@56 byte-identical, all three
  re-simmed at or above their previous receipts. Every kill criterion
  held: K75-1 (E3 +0.3%), K75-2 (1 + 2 ≤ 6 + 3), K75-3 (no regression),
  K75-4 (the constant carries E0/E1/E3 and the three re-bless receipts
  in this log). RFC-072 residual (a) closes; RFC-073's retention
  contract is discharged (the census was consumed). En route: the grid
  composer learned that its inter-strip clearance is the harness's rig
  space (`interior_rig_lanes`, `stamp_bridge_columns`,
  `repair_pole_network_with_keepout`) — a pre-existing latent clash the
  re-quantization exposed. **Residuals, none gating:** (a) PU-from-ore
  stays at 89.9% — its remaining deficit is the tap-fed rows' tail
  starvation (RFC-069's provisioning class; the head-slow half was
  this RFC's and is gone); (b) the validator's `inserter-throughput`
  checks and the DI-bridge credit in `belt_flow.rs` still read the
  flooded `machine_feed_rate` for long-handed pickups — report-only
  today, and a validator-trust change with its own review when it is
  made; (c) the tail-of-row hand is derated with the rest (one extra
  long-handed inserter per affected row end) rather than exempted —
  the ec15 receipt shows the tail hand does not need it, so a
  position-aware exemption is available if the inserter count ever
  matters; (d) `FAR_PICKUP_FACTOR` is a floor from three fixtures at
  one belt tier (express); a yellow/red far belt is denser and slower
  and may realize more of the credit — E2 was not run because E1/E3
  settled the question, and the constant is conservative in that
