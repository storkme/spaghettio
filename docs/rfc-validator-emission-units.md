...
deltas on changes that fix one bug while introducing another. This RFC
...
contribute to a baseline of N?). This RFC attacks the upstream cause:
...
- Not category bucketing. That's #208 — orthogonal and complementary.
  After this RFC lands, #208's "47 errors = 8 belt-flow + 6
...
4. **Browser eyeball.** For the URL in the Motivation section, the
   ...
   Pre-RFC screenshot saved as kill-criterion ground truth.

5. **Trace events unchanged.** This RFC doesn't add new
