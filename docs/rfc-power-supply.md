# RFC: Power placement under face densification
...
*both* the power and fluid checkers. This RFC proposes: **(0)** introduce the
...
- **Four hand-synced machine lists, two validator blind spots**:
  ...
  - The pattern is systemic beyond this RFC's blast radius: review also found
...
- **Phase 0 — energy-source model + list unification.** The judgment phase,
  ...
  - **0f (added 2026-07-19 on the KC1 re-derivation trip, user-accepted) —
    ...
    Expect the largest golden movement of this RFC (every layout gains or
...
- If Phase 0e's port-face fix cannot be a bounded parameterization of the
  ...
  `docs/rfc-fluid-dual-input-row.md` — **stop**; that is a separate RFC and
...
- *2026-07-19 — **revision 2 accepted by the user** ("i'm happy with this
  rfc"); user intends to set it as the session goal. Work proceeds phase by
...
- *2026-07-19 — **user decision: fold both defects into this RFC as Phase
...
- *2026-07-19 — **Phase 0c decided: KEEP biochamber in the pole trigger; no
  layout change** (decision unit; adversarial review CONFIRM-KEEP). The RFC's
...
- *2026-07-19 — **KC1 re-derivation clause TRIPPED (in spirit) by a systemic
  ...
  validate/ and the inserter-sizing RFC's in-game import anchors were never
...
- *2026-07-19 — **Phase 0e-ii (fulgora) LANDED** (`cbf5017`; adversarial
  ...
  cluster) filed as a tracking issue — out of this RFC's scope.*
...
- *2026-07-19 — **Phase 0f LANDED** (`3ae17e2`/`f0f2822`/`4a64f3c`/`e7c9a79`;
  ...
  been quietly violating the RFC's "poles live off leftover tiles" premise;
...
- *2026-07-20 — **Phase 1 LANDED** (`c91e98f`; light-scoped adversarial
  ...
  Phase 1.** The RFC's "expect moved fluid-row lines" prediction is
...
- *2026-07-20 — **Phase 2 LANDED** (`43c690b` instrumentation + `db2bcb9`
  ...
  **Deviation ruled ACCEPT and recorded**: the RFC's Phase 2 spec was
  ...
  and 2 of this RFC are all LANDED.** Remaining: Phase 3 (design accepted
  and committed, docs/rfc-power-reservation.md) and the trim rider (in
  flight).*
- *2026-07-20 — **Phase 3 COMPLETE** (full arc in
  `docs/rfc-power-reservation.md`: 3a-i, 3a-ii, 3b, 3c). Phase 3 delivered
  **reactive band-widening** that clears all five hard cases — EC@20, EC@60-red,
  ...
  (its 5-row-deep top-edge recirc is beyond medium ±3). The RFC's
...
- *2026-07-20 — **THE WIRES ARC — recorded out-of-band (arc review's biggest
  ...
  own RFC phase, so it lived only in commit messages until now. An **in-game
