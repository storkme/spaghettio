...
| `rfp-remove-corridor-template.md` | Implemented — corridor-template pre-pass removed (`fcec676`); junction solver is now the only path for crossings. |
| `rfp-streaming-reconciliation.md` | Implemented — streaming renderer reconciles to `layout.entities` at finish (`d845e56`). |
| `rfp-veto-directed-growth.md` | Implemented — walker-informed growth policy in the junction solver (`1ef2a7f`). |
| `rfp-multi-fluid-rows.md` | Implemented — stacked-T multi-fluid input rows (`c3a9cbb`). |
