# RFC: Net-flow solver (LP replacement for the recursive tree walk)
...
- **Honest limits, found in review:**
  ...
  - *Not all vanilla cycles are self-loops.* An SCC census over the
    ...
    problem — **explicitly out of scope for this RFC**. The solver
    refuses them with a typed error (below); a future RFC can take
...
### Cycle policy (all phases of this RFC)
...
- Multi-recipe cycles: rejected with typed
  `SolverError::UnsupportedCycle`, permanently within this RFC's
  scope.
...
2. **Determinism.** If the 290-item double-run byte-comparison fails
   ...
   also fails, kill the RFC — the approach conflicts with a
...
4. **Perf.** Native, full 290-item sweep: median solve ≤ 2ms and max
   ≤ 10ms (the RFC's own claim is µs–low-ms; missing the claim by
...
`rfc-streaming-reconciliation`'s retracted criterion — hence trigger,
not auto-kill.)
...
- **Phase 2 — layout follow-through**: fluid surplus to perimeter;
  ...
  Each layout piece gets its own design pass; this RFC commits only to
...
- *2026-07-11 — **Phase 2(c) self-loop rows LANDED** (three subagents:
  ...
  or its own RFC). Browser eyeball of the kovarex row is outstanding
...
- *2026-07-11 — **fluid-ingredient self-loop rows LANDED**
  ...
  **coal-liquefaction explicitly DEFERRED** by design review: it needs
  ...
  faces). Kept typed-refused; a proper RFC if ever wanted — it's a
