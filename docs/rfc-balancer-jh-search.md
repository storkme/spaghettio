# RFC: Smarter junction-height search in `compose_series`
...
- The "smarter-jh-search follow-on" was flagged in the spatial-pruning
  RFC decision log (entry by pruner) as the next dominant cost.
...
- **Eliminate the climb entirely via offline lower-bound
  ...
  jh). Out of scope; would be a follow-on RFC if SHORT/LONG isn't
  enough.
...
- **Total (4, 9) wall time regresses** vs the post-spatial-pruning
  ...
  retry path is hit too often. Tune SHORT upward, or accept the RFC
  is wrong.
...
- *2026-05-02 — implemented and accepted (ug-plumber / team-lead).
  ...
  **The 120s baseline in this RFC was an outlier.** The original data
  ...
  **Decisive win on the infeasible climb:** jh=5..8 together took 30.3s
  ...
  each. That was the actual purpose of this RFC and it delivered.
