# RFC: Demand-aware lane provisioning and splitter flow modeling
...
suffice. This RFC runs one decisive ground-truth experiment to
...
2. Record the verdict in this RFC's decision log. It picks the
...
1. **Phase 0 contradiction**: if in-game ground truth contradicts
   `docs/factorio-mechanics.md`'s splitter rules, STOP this RFC
...
5. **Scope fence (evidence gate)**: if utility@10/s's
   ...
   record it as its own finding and do NOT expand this RFC to chase
...
- *2026-07-12 — **Phase 0 resolved without the in-game run, and the
  RFC gains a mandatory companion.** (a) Splitter backpressure
  ...
  must PASS" gate transfers to the follow-up layout-side RFC
