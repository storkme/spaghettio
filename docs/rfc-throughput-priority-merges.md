# RFC: Throughput-priority merges (audit phase)
...
This RFC covers **phase 1 only**: build a classifier that takes any
...
are out of scope here and will be scoped in a follow-up RFC once we have
the audit results.
...
- **Mixed MX1 in the library.** If any library template is classified as
  ...
  *current* library's correctness, separate from this RFC's main aim.
...
5. **Report-driven decision.** The taxonomy report (printed by the audit
   ...
   RFC or pivot per the kill criteria.

## Phasing

- **Phase 1 (this RFC).** Classifier module + audit test + taxonomy
  ...
  what we learned.
- **Phase 2 (out of scope here, next RFC).** Action depends on phase 1's
  ...
  - *No generator work needed* — if MX3 turns out to be the right size
    ...
    ([rfc-balancer-runner.md](rfc-balancer-runner.md)) and we ship.
