# RFC: Modular production — demand-partitioned modules + subtree decomposition
...
not because it's inherently preferred — the point of this RFC is
...
1. **Invalid combinations don't exist.** `subtree_decompose=true`
   ...
   let you express it.
2. **Future strategies slot in cleanly.** The `escargio` RFC
...
templates, so it's not an open problem — but the RFC shouldn't
...
1. **Pooled-strategy regression**: every e2e test asserts snapshot
   equality under `strategy: Pooled` with the pre-RFC baseline.
...
- *2026-04-24 — RFC drafted. Brainstorm preceding this doc
...
- *2026-04-24 (revision pass) — Second-pass review against the draft
  ...
  - *Noted that escargio (folded layouts, separate upcoming RFC)
