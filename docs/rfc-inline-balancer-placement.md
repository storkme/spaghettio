...
2. **Mark the footprint as `Occupancy::Template`** in
   ...
   `HardObstacle`; this RFC just feeds balancer rects into it.
...
This RFC needs more validation than I've done — the kill criteria are
...
- **K1.** If `BalancerInlineFallback` fires on more than 30% of the e2e
  ...
  outcome: opportunistic-only inline; not what this RFC scopes for.)
