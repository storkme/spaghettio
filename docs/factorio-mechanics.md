...
- **I5.** Inserters interact with belt lanes: an inserter dropping onto a belt places items on the **far lane** (the lane farthest from the inserter). Geometrically, the drop lane is determined by the dot product of the inserter's approach vector and the belt's perpendicular: positive → right lane, negative → left lane. *(Matches the game and `common::inserter_target_lane` — this rule previously said "near lane", which was wrong.)*
