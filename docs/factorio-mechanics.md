...
## Belt merger taxonomy (m→n splitter networks)

Splitters compose into networks that route m input belts onto n output belts ("balancers" in community parlance). Not every m→n network is equivalent — they form a strict hierarchy of three behavioral classes, and only the strongest two are useful for asymmetric bus tap-offs.

- **MX1. Throughput-limited.** Outputs may starve or strand items even when the aggregate input/output capacity supports the flow. Trivial cascade or pairwise-merge constructions typically land here. *Diagnostic*: there exists a saturated input vector and unblocked output vector for which the realized output is less than min(m, n). **Not safe** to use as a generic m→n connector.

- **MX2. Throughput-unlimited.** Two sub-properties, both useful:

  - **MX2a. Saturation + balanced rate.** Under fully-saturated inputs and unblocked outputs, every output runs at exactly `total_input / n`. Equivalently: for any *input* k-subset S, max-flow(S → all outputs) = min(|S|, n). Stops short of the full max-flow property — does *not* guarantee re-routing under partial output blockage. **Sufficient for spaghettio's bus** (homogeneous consumer rows mean output blockage is uniform, never asymmetric).
  - **MX2b. Full max-flow.** MX2a + the dual: for any *output* k-subset T, max-flow(all inputs → T) = min(m, |T|). Inputs reroute around blocked outputs through sibling paths so total throughput is preserved when consumer demand is asymmetric or partially blocked. Required for mixed buses where downstream consumption rates differ across rows.

  *Diagnostic*: max-flow between every input-subset (MX2a) and every output-subset (MX2b) pair equals the matched-subset bound. **MX2b strictly subsumes MX2a; MX3 strictly subsumes MX2b.** Asymmetric `(m, n)` MX2a constructions can be 3-5× smaller than MX3 — the throughput-priority generator in [`docs/rfc-mx2-merge-generator.md`](rfc-mx2-merge-generator.md) targets MX2a explicitly.

- **MX3. Balanced (composition-balanced).** Stronger than MX2b: every output belt's content is exactly 1/m drawn from each input belt, regardless of partial saturation. Required only when belts carry mixed item types and downstream consumers depend on the mix (e.g. mixed-bus science loading). *Diagnostic*: the linear flow-rate system, with each splitter modeled as a 50/50 distributor, produces output rates that are uniform combinations of the input rates. **Strictly subsumes MX2b.** Significantly larger than MX2 for asymmetric (m, n).

- **MX4.** For single-recipe-per-trunk bus layouts (spaghettio's design), every belt carries one fungible item type. **MX2 is sufficient**; MX3's composition guarantee is not load-bearing. The current `balancer_library` is generated by Factorio-SAT and is *intended* as MX3, but per-template classification has not been audited — see [`docs/rfc-throughput-priority-merges.md`](rfc-throughput-priority-merges.md).

- **MX5.** Lane-level caveat: MX2 at the *belt* level does not automatically imply lane-level throughput preservation. Standard splitters preserve lane assignment per output (S4), so a belt-level MX2 network is typically also lane-throughput-preserving, but networks that rely on sideloads or lane swaps to merge flows can strand items on one lane while leaving the other half-empty (B5, B8). Lane-level throughput should be checked separately for any new construction.

- **MX6.** Wiki reference: [Balancer mechanics](https://wiki.factorio.com/Balancer_mechanics) — distinguishes "throughput-unlimited" (MX2) from "balanced" (MX3) and describes the universal-balancer back-loop pattern relevant to MX2-with-back-pressure-routing.
...
- **F4.** **Pipe-to-ground** occupies 1x1 and has a facing direction. It connects underground to the nearest pipe-to-ground of opposite facing on the same axis, within a max ENTITY-TO-ENTITY distance of 10 tiles (gap ≤ 9 — `max_underground_distance: 10`, belt `max_distance` semantics). **Sim-measured 2026-07-24 (#407): an 11-apart pair carries nothing** — the old reading of "10 tiles" as the gap was off by one.
- **F5.** Pipe-to-ground has one surface connection and one underground connection on opposite sides. **Convention split (#364, sim-measured 2026-07-22):** the ENGINE's `PlacedEntity.direction` is the *underground* side (surface opposite facing) — this is the convention every rule below and the validator use. The GAME's blueprint-JSON `direction` is the *surface-opening* side, i.e. exactly 180° flipped; `blueprint.rs` flips on export and `blueprint_parser.rs` un-flips on import, the same artifact-boundary treatment as the inserter pickup-side quirk (I5 note). Before the flip existed, every exported PTG pair faced mouth-to-mouth in-game, never paired underground, and severed its fluid network — while validating clean under the self-consistent internal convention.
...
- **P4.** **Medium electric pole**: **1x1** footprint (NOT 2x2 — the 2x2 poles are the substation and the big pole), 7x7 supply area (centered), wire reach 9 tiles. Footprint center is `+0.5` on each axis; a 2x2 pole's is `+1.0`. Supply-area distance (center-to-edge) is 3.5 for a medium pole, 9.0 for a substation; wire reach (`maximum_wire_distance`) is 9 / 18 respectively — these are *different quantities*, do not conflate.
- **P4a.** **Big electric pole**: 2x2 footprint, 4x4 supply area (center-to-edge 2.0 — tiny but real, it is NOT a wire-only pole), wire reach 32 tiles. Quality scales it by the same rules as medium/substation: +1 supply radius per level (4x4 → 6/8/10/14x14) and +2 wire reach per level (32 → 34/36/38/42). *(Wiki-verified 2026-07-24; the engine never places big poles — this row exists for corpus analysis of community blueprints.)*
