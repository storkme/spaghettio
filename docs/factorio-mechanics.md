...
- **MX2. Throughput-unlimited.** Two sub-properties, both useful:
  ...
  *Diagnostic*: max-flow between every input-subset (MX2a) and every output-subset (MX2b) pair equals the matched-subset bound. **MX2b strictly subsumes MX2a; MX3 strictly subsumes MX2b.** Asymmetric `(m, n)` MX2a constructions can be 3-5× smaller than MX3 — the throughput-priority generator in [`docs/rfp-mx2-merge-generator.md`](rfp-mx2-merge-generator.md) targets MX2a explicitly.
...
- **MX4.** For single-recipe-per-trunk bus layouts (spaghettio's design), every belt carries one fungible item type. **MX2 is sufficient**; MX3's composition guarantee is not load-bearing. The current `balancer_library` is generated by Factorio-SAT and is *intended* as MX3, but per-template classification has not been audited — see [`docs/rfp-throughput-priority-merges.md`](rfp-throughput-priority-merges.md).
