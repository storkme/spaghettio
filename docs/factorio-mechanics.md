...
- **MX2. Throughput-unlimited.** Max-flow property: for any matched k-subset of inputs and any matched k-subset of outputs (k ≤ min(m, n)), the network can route k belts of throughput between them. Equivalently, no proper subset of inputs is starved by a downstream blockage that has alternative routing capacity. *Diagnostic*: max-flow over the splitter graph between every k-subset pair equals k. **Predictable degradation under partial input/output**, no composition guarantee. This is what bus tap-offs actually need.

- **MX3. Balanced (composition-balanced).** Stronger than MX2: every output belt's content is exactly 1/m drawn from each input belt, regardless of partial saturation. Required only when belts carry mixed item types and downstream consumers depend on the mix (e.g. mixed-bus science loading). *Diagnostic*: the linear flow-rate system, with each splitter modeled as a 50/50 distributor, produces output rates that are uniform combinations of the input rates. **Strictly subsumes MX2.** Significantly larger than MX2 for asymmetric (m, n) and the source of the coprime barrier in [#136](https://github.com/storkme/fucktorio/issues/136).
