# RFC: Lane-aware validation of baked balancer templates
...
[`docs/rfc-throughput-priority-merges.md`](rfc-throughput-priority-merges.md)).
...
The phase-1 audit in `rfc-throughput-priority-merges.md` (decision log
...
- **No existing template in the library fails the new validator
  ...
  this RFC delivers no actual safety improvement — every template is
...
- *2026-05-02 — drafted. Awaiting approval. Can run in parallel with
  spatial-pruning RFC (different files: Rust orchestration here,
...
- *2026-05-02 — Phase 1 implemented. Module landed at
  ...
  **Triage — kill criterion fired (>30% error rate).** Per the RFC, this
  ...
  3. **Underground-belt sideload-into-input warnings (53 across 22
     ...
     lane caveat" (RFC throughput-priority decision log 2026-05-01).

     These warnings are exactly what the RFC predicted: "every new
