...
| `dropped-connection` | E | yes | the router reports the tile where materialisation deleted a belt and no crossing zone replaced it. **Not an inference** — unlike `belt-flow-reachability` (hole 4) it does no graph reasoning, so a report here IS a hole by construction and earns Error on bus layouts. Added with the #652 column-foreignness fix, whose severed ac7-HS iron-plate tap shipped as a ZERO-ERROR layout |
...
| `belt-flow-reachability` | E spaghetti / **W bus** | yes | the #520 check, rewritten per-tile after incident ten; still cannot block a Bus layout — hole 4. Promoting it to Error was tried and reverted 2026-08-17 (`stress_electronic_circuit_35s_from_ore` went 4 errors → 92; 12 sampled all walked 14–62 tiles to real sources). For the severance case specifically, `dropped-connection` is the exact instrument and this one is the wrong tool |
| `inserter-throughput` | W | **no (demoted 2026-08-14, #632 B6)** | hand-capacity model; never sim-anchored — which is why it no longer steers selection. Demotion receipts: plain suite 1238/1238 green; 513-config fingerprint sweep flips 7 knife-edge configs by ±2–3 entities, each selecting identically when rebuilt standalone, calibrated channels unchanged |
| `inserter-item-throughput` | W | **no (demoted 2026-08-14, #632 B6)** | never sim-anchored in the steering direction. One positive-direction receipt exists — big-electric-pole@1/am2, where its single warning is the only validator signal on the 0.51/s layout (hole 2 narrative) — but that config's protection is RFC-059's `Downstream` default + its teeth test, not this count; the teeth test stayed green across the demotion |
