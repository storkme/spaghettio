# RFC: CP-SAT placement engine for (n, m) balancer graphs
...
handles `(1, 1)`. This RFC covers the placement model itself: the
...
Decoupling synth from placement (the pre-condition for this RFC)
...
6. **Full e2e** — `cargo test --manifest-path crates/core/Cargo.toml`
   ...
   is unaffected unless we explicitly wire it in (separate RFC).

## Decision log

- *2026-05-01 — RFC drafted. Subprocess plumbing landed in `5687ddf`;
  this RFC scopes the placement model itself. Phase 1 starts when
  approved.*
