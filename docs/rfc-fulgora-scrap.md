# RFC: Fulgora scrap-based layouts
...
2026-07-11 spike (see `docs/rfc-solver-net-flow.md` decision log)
...
places) with value-recovery recycling activating for free. This RFC
...
Concrete target the RFC must make real: `electromagnetic-science-pack`
...
- **D2b — multi-solid-output row belts** (not small, inherited): a row
  ...
  nothing to read for the second stream. This is the netflow RFC's
...
The RFC does not pick (a)/(b)/(c) — Phase 0 does, with the decision
...
2. **Sorter viability (pivot within scope)**: if neither generative
   ...
   **Sorter kill (actual)**: if (c) ALSO fails — no pre-solved block
   ...
   that would make us abandon the RFC, stated up front.
...
- **Rate math.** `crafting_speed: 0.5`. `scrap-recycling`:
  ...
  **6 recyclers** (bare arithmetic, no modules/margin); red belt 12;
  blue belt 18. (Aside, not load-bearing for this RFC:
...
description) — 4× the RFC's 1-yellow-belt benchmark rate.
...
them) but the RFC's stated con holds on paper: a 12-item cascade needs
...
proven at scale by the corpus, comfortable headroom at the RFC's
...
- **KC2 (sorter viability) — does not trip.** Architecture (a) fits
  the frozen benchmark with large headroom at the RFC's reference
...
- **Phase 0 outputs are artifacts**: recycler physicals documented in
  this RFC; sorting architecture decision + community-corpus footprint
...
- *2026-07-11 — drafted, following the Fulgora data + solver spike
  (see `docs/rfc-solver-net-flow.md` decision log for the spike's
...
- *2026-07-11 — PR #306 review round: (1) D2 split into D2a (merger
  ...
  un-stranding actually lives there, inheriting the netflow RFC's
...
- *2026-07-11 — Phase 0 complete, both artifacts recorded above
  ...
  sub-phase before Phase 2/3 template work, not a kill of the RFC.
  ...
  scale; a paper design at the RFC's 1-yellow-belt benchmark rate
...
- *2026-07-11 — Phase 3 PARTIAL LANDING; **RFC closes here**. What
