...
| RFC-067 | 2026-08-10 | [`rfc-067-cell-interface-db.md`](rfc-067-cell-interface-db.md) | Proposed — in-repo searchable store of production-subtree implementations keyed by demand motif (counts stored, rates derived; constraints derived from entity vocabulary, never declared). Motivated by the Phase-0 scoreboard: top 5 motifs = 87.7% of machine mass; fabric median 17.8%, so *Phase-0's pre-registered fabric kill criterion* (the RFC-057 lesson) did not fire — RFC-067's own criteria are K67-1..4. Three consumers: interface-first preview, template candidates (inert, under standing never-worse + sim-anchor gates), standing regression corpus. K67-3 makes the null result (no template wins at current density) an explicit live outcome. Tracking: #619 |
