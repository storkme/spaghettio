...
| RFC-065 | 2026-08-04 | [`rfc-065-connectivity-ir.md`](rfc-065-connectivity-ir.md) | Phase 0 in progress — derived connectivity graph over `LayoutResult` (single canonical derivation reusing `belt_flow` primitives), index-keyed topology diff as the transform-admissibility primitive, record-integrity pass (`effective_rows` bands, coordinate-bearing `segment_id` anchors, `power_wires` index sanity). Motivating live bug: compaction/fold never remap `effective_rows`, so rate attribution on compacted layouts fails open against a stale ledger — fixed under this RFC (strips/cuts remap; folds clear). Enabling work for RFC-064 Phases 3–5; movable components (Phase 3) and port-to-port fabric (Phase 4) explicitly deferred to their own RFCs |
