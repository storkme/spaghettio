...
| RFC-067 | 2026-08-10 | [`rfc-067-cell-interface-db.md`](rfc-067-cell-interface-db.md) | Adjudicated at open — in-repo searchable store of production-subtree implementations keyed by demand motif (counts stored, rates derived; constraints derived from entity vocabulary, never declared). Motivated by the Phase-0 scoreboard: top 5 motifs = 87.7% of machine mass; fabric median 18.3%, so *Phase-0's pre-registered fabric kill criterion* (the RFC-057 lesson) did not fire — RFC-067's own criteria are K67-1..4, and the three phase-gating criteria (K67-1..3; K67-4 restates standing constraints) are ADJUDICATED (decision log): **K67-1 CLEAN** (zero escape hatches on the top-5 engine seeds, after the multi-port contract amendment), **K67-2 KILLED** (kill clause fired: six adjudication runs 31.9–32.3% vs the 30% bar — the preview CONSUMER is killed per the criterion's letter; module + calibration instrument remain as baseline), **K67-3 NULL on 3 of 3 realizable motifs** (engine-derived seeds tie the engine at matched demand; ec/ac need the parked multi-group stamping — Phase 3 parked; reopening path = community donors + count ladders). Store + seeds + harness land in #620/#621, sequenced immediately after this RFC (620's review required the design authority on main first). Tracking: #619 |
