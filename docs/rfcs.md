...
| RFC-063 | 2026-07-31 | [`rfc-063-compaction-primitives.md`](rfc-063-compaction-primitives.md) | Design — the funded follow-up to the 2026-07 compaction retrospective (`compaction-retro-2026-07.md`); three primitive-level attacks on the logistics floor under sim-anchored never-worse gates: Phase A (#135 balancer/template shrinking), Phase B (wide-band reshaping spike), Phase C (DI-aware packing spike, revisits RFC-058's own bar on different inputs). Explicitly closes whole-factory/post-pass repacking until a primitive change moves the floor |
