...
| RFC-059 | 2026-07-30 | [`rfc-059-di-coupling-assignment.md`](rfc-059-di-coupling-assignment.md) | **Closed — decided, the claim order is SEARCHED.** Split out of #473. Corpus sweep over 3 machine tiers: 179 targets contend and neither fixed order dominates (downstream-first is better on 6, worse on 2), so `DirectInsertionCandidate` builds both and keeps the better. P2/P3 dropped — no per-target assignment beats the search anywhere. The motivating `rail` case never contends |
