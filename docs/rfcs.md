...
| RFC-059 | 2026-07-30 | [`rfc-059-di-coupling-assignment.md`](rfc-059-di-coupling-assignment.md) | **Closed — decided: tie-break stays P0.** Split out of #473. Corpus sweep over 3 machine tiers: 179 targets contend, neither fixed order dominates (downstream better on 6, worse on 2), so a two-arm `Search` was built and measured better than both. Held back as non-default: on `display-panel@1`/am1 it ships a validator-clean DI cell that sims at **0/s** where P0's native measures 1.00/s (#520). P2/P3 dropped — no per-target assignment beats the search. The motivating `rail` case never contends |
