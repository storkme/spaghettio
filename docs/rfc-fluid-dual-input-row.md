# RFC: Fluid dual-input row geometry (#68)
...
machine-to-machine fluid chain in three columns. This RFC enumerates
...
see `docs/archive/rfc-multi-fluid-rows.md`); the 0-solid + 1-fluid
...
RFC except where the recommended geometry happens to subsume it.

## Design

This RFC is **research only**. No code is changed by landing it. The
...
- **Invariants strained**:
  ...
  - **Trunk fanout**: one fluid trunk fanning out to K vertical
    ...
    capacity is already a known issue (`docs/rfc-junction-solver-
...
- **Geometry validity**: valid in the game. Practically infeasible
  ...
  side effect. **Reject for #68; revisit only if a separate RFC for
...
4. **Multi-fluid is cleanly separable.** Option B handles 2 solid +
   ...
   (`docs/archive/rfc-multi-fluid-rows.md`) handles 0 solid + 2
   fluid via stacked-T. The two RFCs are orthogonal and compose:
...
1. **`tier4_advanced_circuit_from_ore_am2` does not reach a
   ...
   junction-solver capacity wall). Conclusion: this RFC is not the
...
- **Template duplication risk vs. Option D's "fix everything at
  ...
  is a separate RFC. Resist scope creep: Option B is the minimum
  viable fix.

## Out of scope / deferred

- **Multi-fluid recipes (≥2 distinct fluids)** — covered by
  `fluid_multi_input_row` and `docs/archive/rfc-multi-fluid-rows.md`.
...
- **Filter inserter / lane-mixing infrastructure** — tracked as a
  separate prospective RFC (Option D above).
...
- **`tier4_advanced_circuit_from_ore_am2` un-ignoring** — the
  ...
  un-ignore is gated on both RFCs landing.
- **Updating CLAUDE.md tier-ladder language** beyond a one-line
  cross-reference to this RFC — the ladder reflects the *current*
