# RFC: Decomposition-search — strategy-as-candidate selection layer

> **Status**: Phase 0 in progress (this RFC + scaffolding land together).
...
Replace the fixed-strategy menu in [`docs/rfc-modular-production.md`](rfc-modular-production.md)
...
This RFC **does not** propose a new way to lay out machines — it proposes a
...
- **`DirectInsertion`** — for tight producer/consumer pairs with clean
  ...
  - **Layout impact**: requires the placer to understand "these two
    ...
    Likely the largest non-scaffolding chunk in this RFC.
...
- **Phase 3**: `DirectInsertion` candidate. Substantial new placer
  ...
  scoped as its own phase deliberately. Likely needs its own RFC
...
- *2026-04-29 — RFC drafted alongside Phase 0 scaffolding. Scope
...
- *2026-04-29 — Belt-tier escalation removed from candidate catalogue.
  ...
  as the modular-production RFC's "no silent strategy downgrade".*
...
- *2026-04-29 — `DirectInsertion` added to the candidate catalogue as
  ...
  its own phase with its own RFC expansion at land-time.*
...
- *2026-04-30 — Phase 1 landed (`ModuleSizeSplit` candidate, k=2). Key
  ...
  - *Search is **sequential, not parallel**. Native runs first; if its
    ...
    candidate, pick best by density" framing the RFC committed to.
  ...
  - *Acceptance check is `count_missing_balancer_template_warnings(layout)
    ...
    targets exactly the (n, m) coprime trap this RFC exists to
    address.*
