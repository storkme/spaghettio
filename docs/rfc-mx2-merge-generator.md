# RFC: MX2 merge generator (phase 2 of throughput-priority merges)

## Summary

Phase 1 ([`rfc-throughput-priority-merges.md`](rfc-throughput-priority-merges.md))
...
This RFC scopes phase 2: build a Rust-side MX2 merge generator that, for
...
2. Generate templates Factorio-SAT struggles with — coprime
   ...
   without the SAT-runtime cost of [`rfc-balancer-runner.md`](rfc-balancer-runner.md).
...
4. **e2e regression.** Run the full layout suite. Any bus that
   ...
   generator is additive in this RFC — we don't replace existing
   library entries yet).
