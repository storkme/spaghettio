# RFC: Render static layout via ParticleContainer + texture atlas
...
1. The icon on the machine's inserter, showing what's being delivered
   in or extracted out.
2. (Future, out of scope for this RFC) a richer hover tooltip on
...
- **In this RFC**: `getItemIconTexture(item)` returns the cached
...
- **Scrub-mode regresses.** If a scrub-mode rewind / forward step is
  ...
  to start; framerate during drag must stay within 10% of pre-RFC
...
- OffscreenCanvas. Separate RFC if main-thread cost is still an
...
- *2026-04-25 — **Kill criterion #1: PASSED with three orders of
  ...
  → none. The render pipeline is no longer the bottleneck. RFC
  goal achieved.*
- *2026-04-25 — **Follow-ups identified post-merge:** (a) belt
  ...
  small follow-up PRs, not architectural concerns. RFC closes
  here; future renderer optimisations get their own RFCs.*
