# RFC: Logical-graph + placement-only balancer solver
...
reason `rfc-balancer-runner.md` exists. If we hand the solver a *known*
...
Phase 1 audit ([`rfc-throughput-priority-merges.md`](rfc-throughput-priority-merges.md)
...
- Wait for the SAT-runner work ([`rfc-balancer-runner.md`](rfc-balancer-runner.md))
...
still a meaningful win — closing the rfc-balancer-runner work without
...
- ~~**Where does the generator's output go in `balancer_library.rs`?**~~
  *Resolved 2026-05-01:* sibling `balancer_library_extra.rs`. Keeps the
  ...
  fallback).
- **What about the existing rfc-balancer-runner work?** That work
...
- *2026-05-01 — phase 3.1 spike result: `cp_sat` Rust crate failed to
  ...
  subprocess as documented in the RFC fallback. `pip install ortools`
