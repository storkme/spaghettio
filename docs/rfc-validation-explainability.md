# RFC: Validation explainability — from warning soup to visible causes
...
at stamp time is computed and then thrown away. This RFC adds four
...
**Adversarial review proved both humans wrong — which is this RFC's
...
Three failure modes, three causes; two humans and the original RFC
...
to this" — belongs to the face-allocation north-star). This RFC makes
...
- *2026-07-14 — **Phase 2 LANDED (981f40e)**. KC1 anchor re-check
  ...
  **Finding: limits read 24× "geometry", correcting the review's
  ...
  tooling sharpened its own RFC's diagnosis on first use — third
...
- *2026-07-15 — **phantom-event root cause CORRECTED (three theories,
  ...
  landed (571cbe0) — ALL RFC PHASES COMPLETE.** The candidate-leak
  ...
  feedback_user_validates_ui); then this RFC closes.*
...
- *2026-07-19 — **first material payoff + one residual**. The
  `geometry` verdict this RFC's attribution surfaced (far-side cap at
