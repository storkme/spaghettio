...
- **`ec30-am2-ore` at production defaults is unjammed but NOT CLEARED by the validated (3,2) restore (#701, 2026-08-22).** `electronic-circuit@30` on `assembling-machine-2` from ore, with the belt tier left to the engine and `LayoutOptions::default()`, now builds with **0 validator errors** and the meter records **25.0/s vs 30 planned (83.3%, −16.7%)**, converged. That meter reading is a below-plan reading to be believed, not clearance: the restore removes the earlier sim-confirmed 0/s jam by meter evidence, while the residual 25/30 throttle remains an open follow-up. This production-default fixture remains distinct from the stress-EC fixtures below, which force `Some("transport-belt")` through `run_e2e`.
  - **Sim anchor attempted, no delivered-rate receipt available:** the tracked export for `ec30-am2-ore` succeeded (2,071 entities, 0 validator errors), but `cargo run -p spaghettio_sim_harness -- run ... --warmup 432000` exited 1 before Factorio launched with `binding ephemeral port: Operation not permitted (os error 1)`. No electronic-circuit delivered rate or stage breakdown is recorded from that attempt; a successful sim anchor remains required.
