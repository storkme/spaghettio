...
|---|---|---|
| RFC-072 cell interface (Phases 0–2) | 2026-08-25/26 | COMPLETE — Phase 0 re-anchored by the sim (the "seam cost" was the meter's turn-path artifact); Phase 1's two silent-deficiency defects fixed (#727 tap repair-or-refuse, #728 capacity-aware merger; specimen 44.8 → 90.00/90.00 PASS); Phase 2: quantum 40 + the harness drain-power fix + honest registry/bank re-bless (#730; ac45 non-converged 63.7% → PASS 99.6%), harness rig ladders banded/clustered with structural guards (#731; found #732), the K_MAX successor grid composer + record-aware validators + quantizer margins (#733: ec@240 = 2×12, sim 240.00/240.00 PASS, all 1,200 machines working; native at 240/s panics). Rates past the wall now ship via the cell-chain grid up to K ≤ 48 (4×12 strips) at quantum 40. Phase 3 not committed. Residuals: RFC-049 P3 ladder margin, delivered-vs-produced width/kit sensitivity, #732, the untested mega/fluid grid path |
| RFC-075 pickup-side far hand | 2026-08-27 | COMPLETE — the long-handed PICKUP credit (2.40/s at L2) was calibrated on a flooded express belt, which is a row's far belt only at its dead-end tail; a hand with consumers downstream picks from a moving stream. Phase 0 (ec@12 cell, the ec@240 grid's constituent, −7.1%): producers blocked at `full_output` on both belts while the five EC machines craft 2.11 → 2.48/s in belt order — same hand, same credit. E1 (harness-flooded twin): +4.2%, all five at 2.50. E3 (two far hands): +0.3%. Shipped `FAR_PICKUP_FACTOR = 0.85` on the reach-2 pickup credit (`inserter_ladder::far_pickup_rate`; the composer's grid-only `HAND_MARGIN` retired; RFC-073's census re-prices through it). Priced: 1 bank row + 2 registry rows move, every sub-K_MAX strip and ac@56 byte-identical; re-blessed by sim — ec@240 re-quantizes K=24 → K=20 (2×10 of the 5-machine 12/s cell) and sims 240.00/240 from ore, 250/240 from plates; PU-from-ore 87.7% → 89.9% (its remaining −10% is the tap-fed rows' TAIL starving — RFC-069's class; the head-slow half is gone: those rows went from `2.26 2.33 2.43 1.32*` to `2.50 2.50 2.50 0.84*`). En route: the grid composer keeps its bridge poles out of the harness rig footprints (`interior_rig_lanes`) — the K=20 grid's bridge sat on a feed rig and the harness refused it. Closes RFC-072 residual (a); discharges RFC-073's retention contract |
| RFC-073 margin in the inserter ladder | 2026-08-27 | CONCLUDED AT PHASE 0 — K73-1 tripped on the census before the margin was written: the ladder was instrumented (`SidePlan::capacity`, `InserterSideSized`, `bus/sizing_census.rs`, two ignored probes) and both receipted corpora censused — at-plan bank rows ship input hands at 0.93–0.974 of the ladder's credit (ec30-partitioned 0.974 → 99.4%), the ec15 cell produces at plan with its last-in-row long-handed hand at 1.042, and a short row (`tier2_electronic_circuit_from_ore`, 93.3%) has no hand above 0.65, so fullness on the credit predicts nothing and a uniform margin would re-shape a dozen at-plan rows for no gain. Replacement pointer for RFC-072 residual (a): the K=18/K=20 grid failures and PU-from-ore's twenty 2.40/2.40 iron sides are serial single long-handed hands on one belt at the credit — a pickup-side (belt density / lane fill) effect to test in the sim; the quantizer's scoped `HAND_MARGIN` stays as the receipted exception. Instrument retained under contract |
| RFC-074 cells at the surface (RFC-072 Phase 3, scoped) | 2026-08-27 | COMPLETE — the composition's registry receipt rode `LayoutResult.warnings`, which the web reads as "do not export", so the browser could not export the ec@240 grid RFC-072 built. Unit 1: typed `CompositionReceipt` (kind, strips, verification, verified) — the note stays on `warnings` because selection counts that channel (K74-1). Unit 2: export restored, composition badge (as loud about FAIL/unverified as PASS), strip outlines. Unit 3: the browser's from-plates ec@240 grid PASS 240.00/240.00; the mega/fluid grid (ac@56 from ore + crude, 2×7 strips, 0 errors) PASS +2.5% — RFC-072 residual (f) closed. Unit 4: uniform K over-builds 0–6.7% chain-wide (median 0.8%) → heterogeneous composition and the library closed by measurement (K74-3). Residuals: no per-strip census in the harness report; the one-warning selection penalty on cell candidates (RFC-071 policy question); user eyeball of the surface |
...
- **Bio/fluid self-loop fixtures unmeasurable in the harness rig (2026-08-01)**: `bacteria_self_loop_regression` measures 0/s (`no_fuel`), same class as pentapod/fish and the `fluid_ingredient_shortage` fixtures (sulfuric/heavy-oil). Validator-clean but sim-dead — a rig + validator-coverage gap; on the sim skip-list until fixed (`rfc064-phase2-followups.md` §2). **Partially superseded (2026-08-28, #461 part (b))**: the biochamber fixtures (`bacteria_self_loop_regression`, `pentapod_egg_self_loop`) are no longer validator-clean — they now carry `burner-fuel` Errors naming the exact unfuelled machine, so "validator-clean but sim-dead" no longer holds for this half; still sim-dead until an actual fuel-delivery feature exists. The fluid-starved half of this bullet (`fluid_ingredient_shortage` on sulfuric/heavy-oil) was fixed separately by #732.
...
- **Deep-chain throughput deficits, sim-measured 2026-08-06 (RFC-064 productivity stack tip)**: with the research-productivity axis correctly declared, `processing-unit@1/s` measures **68.2% of plan** and `advanced-circuit@5/s` **83.3%**, while `electronic-circuit@10/s` is exactly 100% — so the deficit is depth-specific, not general. The signature is a **uniform** ratio across every stage of the chain (PU run: copper-cable .6874, copper-plate .6878, EC .6875, iron-plate .6874, plastic .6875, target .6818) plus ~1/3 of machines in `full_output`, i.e. a throughput/distribution ceiling, NOT a productivity-modelling error — declaring the axis is what made these numbers interpretable, it did not cause them. AC is bit-identical declared-or-not despite a measurably smaller plan — **no longer true as of 2026-08-21** (#689 W1c): `advanced-circuit@5/am2` from ore exports 2134 entities undeclared and **2125 declared** at `plastic-bar=0.10`, same 137×78 dims. The engine has moved since 2026-08-06; re-measure before reusing this equivalence anywhere. **PU root-caused 2026-08-07** — the eyes-first gate paid out exactly as intended: the owner served the fixture, saw the green-circuit belt into the PU rows starved (4 EC producers feeding 8 consumers; the sulfuric-acid backup was a downstream symptom), and the validator turned out to have flagged precisely this — three `input-rate-delivery` warnings naming the starving machines, which `selection_warning_count` was excluding from candidate ranking. With the exclusion lifted, the re-ranked PU layout sims at **102.0% of plan** (converged, 140/145 census working) — the lift was parked because it also re-ranks `stacking_ec_60s` onto a supposedly "physically impossible" winner — **retracted 2026-08-07** (PR #601): that verdict compared an aggregate stamp to one belt's capacity and has zero true positives, so THAT objection is void — but the lift has its own separate status, tracked ONLY in `validator-trust.md` hole 2. **AC@5/s ALSO FIXED by the same lift, measured 2026-08-07: 83.3% -> 99.7% of plan** (4.98/s vs 5.00 planned, PASS, converged, kit clean, 109 machines working) — and every stage lands at plan, not just the target (cable +0.0%, EC +0.0%, plastic -0.3%, petroleum +0.6%). It was never eyeballed; it did not need to be, because the deficit had the same root as PU's. So the deep-chain deficit class is substantially the `input-rate-delivery` exclusion: PU 68.2% -> 102.0%, AC 83.3% -> 99.7%, tier2_electronic_circuit 58% -> 91%. **The stress-EC ~50% pair HAS now been re-measured post-lift (2026-08-07/08)** and it moved the same way: `stress_electronic_circuit_30s_from_ore` ~50% -> **92.1% delivered** and `stress_electronic_circuit_60s_red_from_ore` ~50-51% -> **90.7% delivered** (both kit-clean, converged, warmup 432k; banked at `~/spaghettio-corpora/postlift-2026-08-07/`). So the historical ~50% was overwhelmingly the selection bug, not a throughput ceiling — but a real ~8-10% residual remains on both, and both fixtures have all four stages exactly zero-headroom, which is the shape the bullet below predicts. **2026-08-15 (#644, PR #648)**: the swap-era 140/218 lane-throughput Error readings on this pair were retracted as walker phantom-UG-source artifacts (corpus now 0 lane errors); the phantoms had also steered candidate selection for the three days they existed, and the fix flips both winners BACK to these banked layouts (entity-count-exact 3369/4967), so the 92.1%/90.7% receipts attach to what the fixed engine ships. The residual's fix campaign is RFC-069 (achievable-duty provisioning); its Phase-0 additions: ec22 delivers 99.4% (the 22-vs-30 boundary is real under measurement) and tier5-PU@2 axis-declared delivers 95.6% (the meter's 85.6% was an axis-mismatched export). **RFC-069 resumed 2026-08-24 as the trunk/tap-provisioning campaign — Phases A1+A2 landed (#720, #721)**: ec35/ec40/tier5's blocker re-diagnosed as the coprime balancer-trap class (tier5's "router block" retracted — same class), the `k1-shape-fix` rescue made reachable on the Pooled path, ec35 flips from the 313-error merge-tap at 22.9% to a 0-error k1 layout **sim-measured 93.7% delivered** (kit-flagged; meter 95.7%); A2 adds multi-consumer enrollment (tier5's k1: Refused → Produced/12E) and the held-incumbent migration, which flips **ac45 from sim-dead 0.0/s to an error-free cell-composed layout at 63.7–66.7% non-converged** (a real ~2/3 ceiling — the meter's 97.8% is wrong on the shape; residual joins the cell-drain ledger). **Phase A3 (2026-08-25): the trap class dissolved at the root** — the lane split now consults the stamper's own resolvability oracle and pads unresolvable trunk counts (the "router class" attribution retired: the real defect was zero-height balancer bands from `family_stamp_plan`-Unresolvable shapes). The NATIVE reclaims the class: ec40 flips off the 631-error merge-tap onto a 1-error native, **sim 36.8/40 = 92.0% converged kit-clean** (from 18.5%); ec35's native builds the rescue artifact itself; tier5@0.6 sheds its three trap families. **Campaign COMPLETE 2026-08-25**: Phase B adjudicated GATED by measurement (tier5@0.6 = 13 structural errors for +4.4% — measured-negative; two rescues falsified by experiment; duty stays opt-in with the ec30 99.4%/ec60-red 100.0% receipts standing; forward path = duty-as-candidate + density re-weigh, the RFC's remaining substance) and Phase C shipped (the typed unreachable-rate refusal). ec40's residual 8% is UNDECOMPOSED between zero-headroom (the ec30-family precedent) and the pad's orphan-stub dilution — the duty knob measures byte-identical on the padded natives, so the decomposition needs a stub-less arm (recorded follow-up).
  ...
  - **The METER already catches this class, and nothing consumes its verdict (2026-08-07).** Run on the same tier2 layout it reports copper-cable **28.8/s (96% of plan)** and electronic-circuit **9.6/s (96%)** — against the real sim's 27.0/90% and 9.09/91%. Directionally right on both stages, ~5pp optimistic on magnitude, and **in 19 seconds** versus ~10 minutes for the headless run. It sees it because it models inserter swing and lost swings (`inserter.rs`: *"every tick spent here is a lost swing — the starvation signal"*), which is exactly the duty loss a zero-headroom stage cannot absorb. The gap is not the meter's model: the meter is calibrated AGAINST the sim as a research instrument and is wired into no gate — not e2e, not `selection_warning_count`, not `validate/`. A fast below-plan predictor for the whole deep-chain class exists and is unused. Remaining caveat: the original 68.2% run converged at the *minimum* checkpoint count and is unreconciled with #591's PR note claiming 90–98%. Full entry + repro: `rfc064-phase2-followups.md` §9.
    - **2026-08-26: RFC-072 Phase 1 complete** — the deliver-or-refuse contract now covers tap assignment (repair-or-refuse, #727) and output-merge capacity (ceil-packed tails, #728); the cable-90 specimen that shipped 0-error at −50.2% sim now produces 90.00/90.00 (PASS). Details: [`rfc-072-cell-interface.md`](rfc-072-cell-interface.md) decision log.
    - **2026-08-25: the below-plan direction itself is falsified for TURN-HEAVY fixtures** — the meter manufactures 5–18pp phantom under-reads on composed turn-path layouts that the sim anchors at exactly plan (straight-line fixtures meter clean; capacity-bound failures measure accurately). Sim-anchor before trusting any below-plan meter reading on a layout whose item paths turn. Full entry + the four-fixture calibration set: [`meter-divergence.md`](meter-divergence.md) §2026-08-25 (found by RFC-072's Phase-0 sim-anchor pass).
...
- **[#732](https://github.com/storkme/spaghettio/issues/732) — bare-pipe fluid feed heads carry direction 0; the kit builds their rigs INTO the layout (found 2026-08-26 by #731's rig-vs-layout guard sweeping every measured bank manifest).** The calibration bank's three "non-converged 0.000" rows (`tier3_heavy_oil_cracking`, `tier3_sulfuric_acid`, `tier3_plastic_bar`) were never layout measurements — every other fluid-fed row has a `pipe-to-ground` head with a real direction and converges. The harness REFUSED those three at codegen (loud) instead of reporting a fake non-converged verdict. **FIXED 2026-08-27** — engine-side (a bare-pipe fluid head records the southbound lane flow; `tests/boundary_direction.rs`), the bank re-exported with every blueprint byte-identical and exactly the three manifests changed, and the three rows MEASURED for the first time (converged, kit clean): plastic-bar produced 10.00/10.00 with delivered −1.3% (PASS); heavy-oil cracking and sulfuric-acid are produced-only verdicts — fluid boundaries are uncalibrated (RFC-050 Phase 1), so there is no delivered metric — at 3.0× and 3.75× of plan (light-oil 15.0/s on 5.0, sulfuric-acid 18.75/s on 5.0), the same fluid-target over-production class the advanced-oil rows already show at 1.5×. "Measured" is the claim; the fluid rows are not "at plan". Known-refused set empty; sweep 35/35.
