...
- **stress-EC high-rate half-plan (2026-08-01, RFC-064 Phase 2 sim)**: `stress_electronic_circuit_30s_from_ore` and `stress_electronic_circuit_60s_red_from_ore` measure **~50% of plan** (15.15/15.15 vs 30; 30.5/30.0 vs 60) and `tier2_electronic_circuit` ~42% below (5.77/5.81 vs 10) — **that tier2 figure is STALE as of 2026-08-07**: the `input-rate-delivery` lift re-ranks that config onto a different winner, so 5.77 describes the previous layout, not what ships. Re-measure before quoting. The two stress-EC figures are unaffected on **both native and compact** — a pre-existing solver throughput ceiling for high-rate EC-from-ore, confirmed dynamically by sim (the #519 `input-rate-delivery` warnings predicted it). Not a compaction regression; open defect, see `rfc064-phase2-followups.md` §1.
