...
- **`ec30-am2-ore` at production defaults PRODUCES NOTHING — sim-confirmed 2026-08-21 (#689 W1c).** `electronic-circuit@30` on `assembling-machine-2` from ore, belt tier left to the engine, `LayoutOptions::default()`: the shipped layout is **1991 entities carrying 3 `belt-dead-end` Errors** (plus 76 warnings) and measures **0.00/s at every stage** — target, copper-cable, copper-plate and iron-plate all **−100.0%** — across 4 checkpoints at `--warmup 432000` (2 game-hours) `--speed 32`. Kit-clean, fluid-clean, all 1991 ghosts revived, one pole network; census **150 machines `full_output` + 20 `item_ingredient_shortage`**, i.e. a JAM, not a starve. The meter tripwire's blessed −100%/non-converged row (#693) was correct; this is ground truth confirming it, not an instrument artifact. Not fixed (W1c is a diagnostic track); root cause unchased.
  - **Do not reconcile this against the `stress_electronic_circuit_30s_from_ore` figures further down — they are a DIFFERENT fixture.** That one forces `Some("transport-belt")` and goes through `run_e2e`, i.e. under the harness's two fossil options (`cell_composition: Off`, `inserter_capacity: 0`); its banked artifacts are 3369 entities (dense winner, 92.1% delivered) and 4934 (RFC-069's duty-0.6 / phantom-era sprawl, 99.4%). Neither describes the 1991-entity production-default layout, and the campaign note that "the #644-era anchor had ec30 ≈99.4%" conflated all three — the shipped-default anchor for the yellow-belt fixture was 92.1%, and 99.4% belongs to an artifact behind `planning_duty: 0.6`.
  - **Why it ships green**: no e2e test covers `electronic-circuit@30 / assembling-machine-2` with an engine-chosen belt tier. Every ec@30/am2 case in `e2e.rs` passes `Some("transport-belt")`, so suite greenness has never said anything about this configuration. This is the coverage shape the RFC-070 parity corpus's option-set axis exists to make visible.
  - **Selection shipped it knowingly**: `parity_corpus_baseline.json` records this fixture's winner as `native` at deciding stage `merge-tap` under all five option sets — the `ErrorKinds` path whose own comment says it "still returns the error-laden best rather than refusing".
