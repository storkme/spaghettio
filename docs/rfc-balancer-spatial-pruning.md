# RFC: Spatial pruning in `solve_pure_routing_circuit`
...
- **Per-edge dynamic slack based on edge complexity.** E.g. edges
  ...
  initial RFC; would be a follow-up if a fixed slack proves
  insufficient.
...
- *2026-05-02 — drafted. Awaiting approval. Can run in parallel with
  the bake-lane-validation RFC (this one touches Python, that one
...
- *2026-05-02 — landed (commits `7f840a3` initial impl, `72e9cda`
  ...
  **Verification numbers** ((4, 9) Clos compose,
  ...
  RFC-predicted "30–50% per edge".

  (5, 9) jh=9 standalone: previously **UNKNOWN at 645s timeout**
  (the RFC's headline failing case). With pruning + fallback OFF:
  **OPTIMAL in 324s solver/wall**. Headline unlock met.

  **Why the fallback was harmful in practice.** The RFC rationale
  ...
  **Follow-on flagged.** With pruning landed, the dominant remaining
  ...
  RFC; flagged for whoever picks up the next "going larger" thread.
