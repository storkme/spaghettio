# RFC: Build quality — quality-tier entities in the layout engine
...
user constraint it is today. This RFC is **build quality only**: making
...
separate future RFC (already flagged as a Phase-3 concern in
`rfc-fulgora-scrap.md`).
...
byproducts and validator calibration above this; this RFC is
...
Scope boundary, made explicit (user question, 2026-07-20): this RFC
...
Not free, and **deliberately not in this RFC**: exploiting the larger
...
intends to pick it up **immediately after this RFC lands**. (If Phase
...
4. **Scope fence.** If implementation ever needs per-recipe or
   ...
   split into the future RFC rather than growing this one.
5. **Output-path honesty gate (Phase 2 entry).** Before the legendary
   ...
   fixing it is out of this RFC's scope: file it as its own issue, and
   ...
   after the merger issue closes. This RFC does not ship a fixture
...
- **In-game import anchor** (user-run, same shape as
  `rfc-inserter-sizing` kill criterion 5): import the legendary
...
- **Phase 3 — deferred.** Per-class quality overrides; uniform-
  ...
  this RFC; pulled forward if Phase 2's eyeball shows redundant pole
  bands).
- **Future, separate RFC.** Quality item production (modules,
...
- *2026-07-20 — **accepted** by user. Functional-only stamping
  ...
  returning to it immediately after this RFC. Status → ACCEPTED;
...
- *2026-07-20 — kill criterion 1 verified in-game by the user against
  ...
  with their helpers in Phase 2. Per-tier counts match the RFC's hand
...
- *2026-07-20 — Phase 2 adversarial review (code + contract lenses)
  ...
  that predates this RFC, so it is a legitimate same-host identity
...
- *2026-07-20 — #313 resolved as premise-falsified (user directive:
  ...
  is now unblocked and is the RFC's sole open item.***
