# RFC: lane-aware CP-SAT belt routing
...
### Things explicitly out of scope for this RFC
...
- **Underground belt support.** UGs preserve lanes through the
  ...
  shapes this RFC unblocks (the merger sub-network can reach all
  feedback paths via surface belts on grids ~12×10). UG support is
  phase 4 of the placement RFC.
...
- *2026-05-02 — RFC drafted after `fe09494` reverted the lane-unsafe
  shapes.*
...
- *2026-05-02 — `(1, 5)` deferral re-routed through
  `rfc-lane-safe-synth.md` — but that RFC got tested empirically
  ...
  eliminate. See the decision log in `rfc-lane-safe-synth.md` for
...
- *2026-05-02 — Phase 4 increment 3 (`(1, 5)` placer) parked. The
  ...
  belts (out of scope per the original RFC) there's no way to
  ...
  `rfc-lane-safe-synth.md` track but with different motivation).
