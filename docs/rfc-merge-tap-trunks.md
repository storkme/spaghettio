# RFC: Merge-and-tap trunks — retiring large balanced lane families
...
1,636 validation issues). This RFC replaces large balanced families
...
demand-pull walker (rfc-lane-demand-flow Phase 1, `6849935`) models
...
- **PR #272 / docs/rfc-balancer-place-routing.md (May 2026, closed
...
- **#136 (closed)**: the previous coprime-shape wall, closed via
  ...
  this RFC stalls (the 2026-07 spike's shape census doubles as the
  bake worklist).
- **Uniform output balancing (design fossil)**: producer rows emit
  ...
  This RFC retires the premise's consequences deliberately, not
  accidentally.
...
ghost router's general multi-tap rendering (rfc-unified-belt-specs
...
1. **Phase 0 blast-radius gate**: the shape census (2026-07 spike)
   ...
   RFC is over-engineering — close it and bake shapes instead.
...
5. **In-game anchor (tail tap)**: one flipped layout gets an in-game
   ...
   everything (validator-debt, same standard as the inserter RFC).
...
- Area/entity deltas as FIRST-CLASS evidence at the flip (kill
  criterion 4) — this RFC claims an efficiency win; measure it.
- Predict-then-verify re-blessing (the inserter-RFC discipline):
...
- *2026-07-13 — re-review verification: **SHIP.** All five findings
  ...
  accurate; prior-art attribution corrected). **RFC ready for
...
- *2026-07-13 — **Phase 0 census landed (spike; artifacts committed
  8340f8b). Kill criterion 1 evaluated: the RFC does NOT close — the
  ...
  coverage test documents the gap — folded into this RFC's Phase 3
  ...
  confirmation pending as a follow-up. **RFC ready for user
  acceptance.***
...
- *2026-07-13 — **Checkpoint B: machinery landed DORMANT (499287b);
  ...
  RFC's ghost_router.rs:2182-2264 reference is the flow-unify pass,
...
- *2026-07-14 — **#1c verdict: the 51 are REAL — class B 51/51,
  ...
  15.0 left / 29.86 RIGHT (sideloads fill one lane — the RFC D3
...
- *2026-07-14 — **−8 diagnosis: owner is the lane_planner column
  ...
  anchors (still open from that RFC). The /goal push ends with
