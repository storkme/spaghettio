# RFC: Multi-splitter belt routing for the placement solver
...
[`rfc-balancer-graph-place.md`](rfc-balancer-graph-place.md) framed as
...
The MVP confirmed the *plumbing* works. This RFC nails down the
...
This RFC picks defaults for each decision. They're all overridable.
...
criterion from the parent RFC. UG encoding adds maybe 30% more
...
Per the parent RFC D4 (sibling `balancer_library_extra.rs`):
...
This RFC doesn't change that decision, just implements it.
...
- **`(4, 9)` Clos placement exceeds 250 entities.** Parent RFC's
  kill criterion. Phase 3.0 estimated ~80-120 entities for placement;
  if we land at 250+, the value prop is gone.
- **CP-SAT solve time per shape > 5 minutes.** Parent RFC. If single
...
- **Phase 3.4+** — bake into `balancer_library_extra.rs` per parent RFC.
...
- **Verifier divergence on saturated-merge constructions.** PR #270
  ...
  honest: this RFC's classifier checks the saturated-model invariants
  ...
  verifier becomes authoritative for the placement output, this RFC's
