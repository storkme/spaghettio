# RFC: Inserter sizing — matching machine-side throughput to planned rates
...
(`rfc-lane-demand-flow.md` Phase 1); the 1/s gauntlet now carries
2/8/11/22/42/55 honest warnings across the six packs. This RFC sizes
...
- *2026-07-12 — **Phase 0v2 complete: KC1-v2 PASSES.** 1/593 sides
  ...
  The RFC's cited zero-margin case reproduced to the cent (PU's EC
...
- *2026-07-12 — **CORRECTION: the entry above is SUPERSEDED — KC1-v2
  ...
  failure class this RFC's own delta review flagged in the
...
- *2026-07-12 — **v3 re-census complete. The extension works exactly
  ...
  clearance), a full reach-2 redesign RFC, or accepting the residue
  with honest warnings and re-scoping the definition of done (the
  RFC's own best-effort language anticipates this). With user.***
- *2026-07-12 — **user decision: ACCEPT THE RESIDUE — v3 is the
  ...
  not defects of this RFC. KC1's packs-zero condition is explicitly
  ...
  future RFC merging with the dynamic face-allocation north star
...
- *2026-07-12 — **Phase 2 LANDED** as the KC3-sequenced pair. Step A
  ...
  anchors).*
- *2026-07-12 — **Phase 3 LANDED (6b3815b): the RFC's Definition of
...
- *2026-07-13 — **Phase 4 LANDED (cfc20cb). All four phases complete;
  the RFC remains OPEN on exactly one item: kill criterion 5's two
  ...
  out of this RFC's scope, still the scaling program's next targets.
  ...
  miscalibrated → the RFC's verdict flips from done to
  validator-debt.*
...
- *2026-07-13 — **hardening pass complete** (1906510 / 5656283 /
  ...
  harder cases are release-only and unmeasured. **This RFC is now
