...
- **`factorio exited early`** — lock contention (see above) or a real
  crash: read the log at the path `run` printed. For back-to-back runs
  against one install, guard on the lock:
  `until flock -n <install>/.lock -c true; do sleep 2; done`.
