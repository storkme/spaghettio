...
RFC-070 Phase 2c applies the same rule to `TraceEvent::SelectionShadowCompared`: the reverse v1 shadow was removed after the shipped policy passed the 160/160 parity gate, and the event is no longer emitted. Old local `.fls` files captured during Phases 2a–2b that contain this event will fail to parse after the deletion; regenerate them. The committed snapshots were checked for the deleted tag and remain clean.
