# RFC: Reserved power space (Phase 3 of the power arc)
...
to `docs/rfc-power-supply.md`, whose Phase 3 this specifies. 0f is merged
...
cases). This RFC proposes **reactive, selective space reservation**: when a
...
- **Cost baseline** (landed committed-config table, post-0f): corpus poles
  ...
  parent RFC re-anchors to that census. The two-band+mop-up covers
...
param is fed `&[]` today and is listed for deletion in the parent RFC's 0f
...
- *2026-07-20 — **Phase 3a-i LANDED** (`534f6ad`; adversarial review APPROVE,
  ...
  supply ±9 (18×18), wire 18, substation↔medium min(18,9)=9 — RFC numbers
...
- *2026-07-20 — **Phase 3a-ii LANDED** (`e2fb777`/`5e37826`/`867deae`/`3481841`/`50408a3`;
  ...
  separate fork). The reactive repair mechanism works exactly as the RFC
  designed — but the RFC's CENTRAL PREMISE is FALSIFIED and recorded: it
...
- *2026-07-20 — **Phase 3b LANDED** (kovarex top-edge substation boundary
  variant). The RFC's 3b hypothesis is CONFIRMED shape, and — unlike all four
...
- *2026-07-20 — **Phase 3c LANDED — arc close-out** (re-census + trigger-(b)
  ...
  - **Substations counted for the first time.** `pole_census.py` counted only
    ...
    kovarex.
  - **Substation-necessity premise — final tally.** The RFC's central premise
...
- *2026-07-20 — **chemical@10 accounting closed (arc review).** The reservation
  RFC promised "the gauntlet is run once for the non-gating pair," but the 3c
